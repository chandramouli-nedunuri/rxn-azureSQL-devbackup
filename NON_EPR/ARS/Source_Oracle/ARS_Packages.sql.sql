INSERT INTO ALL_OBJECTS (DDL) VALUES
	 (TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE "ARS"."PKG_PDX_SCHEMA_UPDATER" AS
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "ARS"."PKG_PDX_SCHEMA_UPDATER" AS
...
END IF;
g_config_record.ALLOW_D')||TO_CLOB('OWNGRADE := NVL(get_num(get_value(''ALLOWDOWNGRADE'')),0);
...
BEGIN
RETURN pkg_pdx_schema_updater_meta.get_met')||TO_CLOB('adata_tbl(p_file_or_task, p_parameter, CASE p_source WHEN ''U'' THEN ''B'' WHEN ''A'' THEN ''B'' WHEN ''D'' THEN ''A'' WHEN ''R'' THEN ''A'' ELSE p_source END);
...
BEGIN
BEGIN
SELECT action_code||sta')||TO_CLOB('tus_code
INTO l_status
...
VALUES ( p_version_id, p_error_record.error_message, p_error_record.sql_error_code, p_error_record.sql_erro')||TO_CLOB('r_message );
END IF;
COMMIT;
END log_version_end;
...
l_task_id := log_task_start(p_call_id, p_version_id, p_filename, p')||TO_CLOB('_action);
END IF;
...
WHERE schema_owner = SYS_CONTEXT(''USERENV'',''CURRENT_SCHEMA'')
AND pdx_schema_upd_pkgcall_hist_id = p_call_id
AND pdx_schema_version_history_')||TO_CLOB('id = p_version_id
AND file_name = p_filename
)
WHERE id = max_id;
EXCEPTION
...
JOIN pdx_schema_taskver_meta')||TO_CLOB(' m on (m.meta_tag = ''REQUIRES'' AND task_source = ''A'' AND UPPER(m.task_version_value) = f.task_version)
...
UNION ALL SELECT ''VIEW'' object_type_nam')||TO_CLOB('e, ''SELECT'' privilege_name FROM DUAL
...
SELECT * FROM default_privs where (object_name, p_tag) N')||TO_CLOB('OT IN (SELECT object_name, grantee FROM schema_except)
...
minus
select privilege_name privilege, object_name table_name, u.username gra')||TO_CLOB('ntee
from defined_privs p
left outer join pdx_schema_mapping m on (m.mapping_type = ''USER'' and m.search = p.grantee)
left outer join all_users u on (u.username in (p.grantee, m.replace))
left outer join session_roles r on (r.role in (p.grantee, m.replace))
...
and task_source = ''A''
and ta')||TO_CLOB('sk_version_value = ''SYS''
...
END LOOP;
END replace_')||TO_CLOB('tablespace;
...
l_part := regexp_replace(regexp_substr(dbms_metadata.get_ddl(l_type, l_obj, l_owner),''(PARTITION BY .*?[(]PARTITION|LOCAL .*?[(]PARTITION) .*[)]( *[^ ]* ROW MOVEMENT| *PARALLEL [^ ]*)* *$'',1,1,''n''),''PCTFREE.*? TABLESPA')||TO_CLOB('CE '',''TABLESPACE '',1,0,''m'');
...
IF regexp_count(mrec.search,''^<[A-Z][A-Z0-9_]{1,29}>$'',1) != 1 OR regexp_count(mrec.replace,''^[A-Z][A-Z0-9_]{1,29}$'',1) != 1 THEN
RAISE_APPLICATION_ER')||TO_CLOB('ROR(-20100,''Invalid Mapping -- PDX_SCHEMA_MAPPING_TRG should have prevented'');
END IF;
...
p_ddl_sql_tbl(l_table_index) := trim(l_')||TO_CLOB('sql);
...
IF NVL(p_ac')||TO_CLOB('tion,''X'') = ''A'' THEN
...
JOIN pdx_schema_file_taskver f2 ON (UPPER(m.task_version_value) = UPPER(f2.task_versio')||TO_CLOB('n))
...
JOIN')||TO_CLOB(' file_list v ON (v.type = ''Release'' AND v.file_name = l.version_file_name)
...
')||TO_CLOB('l_value := TRIM(regexp_substr(l_attribcheck(l_idx),''^[^:]*:[   ]*(.*)$'',1,1,NULL,1));
CASE UPPER(l_key)
...
l_tas')||TO_CLOB('k_version VARCHAR2(20) := get_config(''TASK_VERSION'');
...
JOIN pdx_schema_file_taskver USING (task_version)
JOIN pdx_schema_updater_sql USING (file_name)
WHERE meta_tag = ''ROLLBAC')||TO_CLOB('K'' AND task_source = ''A''
AND (action_code != ''R'' OR status_code != ''S'')
...
IF new_rec.action_code = ''A'' AND new_rec.status_code = ''S')||TO_CLOB(''' THEN
BEGIN
...
IF is_same_apply_sql(rb_rec.file_name')||TO_CLOB(') THEN
...
log_task_history (p_call_id, p_version_id, l_task, ''R'', ''F'' , error_record(''Requested task rollback, but rollback is not allowed'', NULL, NULL)')||TO_CLOB(' );
END IF;
l_task := task_list.NEXT(l_task);
END LOOP;
RETURN l_rtn;
...
pkg_pdx_schema_updater_meta.read_metadata;
	  BEGIN
SELECT CASE WHEN target_version IS NULL THEN NULL')||TO_CLOB(' ELSE current_version END INTO l_prior_version
FROM pdx_schema_version_history
...
UPDATE pdx_schema_updater_sql
			...
			END LOOP;
			INSER')||TO_CLOB('T INTO pdx_schema_updater_manifest (version, file_name, apply_order) VALUES (manifest_rec.target_version, manifest_rec.file_name, pdx_schema_master_seq.nextval);
...
FROM pdx_schema_f')||TO_CLOB('ile_taskver r
...
log_task_history ( p_call_id, l_version_id, l_task, t_actio')||TO_CLOB('n, ''W'', error_record(''No action taken: Task status is already applied'',NULL,NULL) );
...
CASE UPPER(l_key)
WHEN NULL T')||TO_CLOB('HEN NULL;  -- Currently, there are no supported ATTRIBCHECK metadata
ELSE
...
ELSE
		              l_rtn := false;
log_task_histo')||TO_CLOB('ry(p_call_id, l_version_id,l_task, t_action, ''F'', error_record(''Requested task rollback, but rollback is done by another task'',NULL,NULL) );
END IF;
ELSE
l_rtn := false;
...
where compare_versions(c_source_version,c_target_version) < 0
and compare_versions(lead_version,c_source_version) > 0
and compare_versions(lead_version,c_target_vers')||TO_CLOB('ion) <= 0
...
')||TO_CLOB('EX_INVALID_CONFIG_KEY          EXCEPTION;
l_rtn             BOOLEAN;
l_undo_rtn        BOOLEAN;
l_pkgcall_id      NUMBER;
l_process_id      NUMBER;
...
END IF;
IF get')||TO_CLOB('_config(''TASK_VERSION'') = p_current_version AND get_config(''APPLY_TYPE'') != ''TASK'' THEN RAISE EX_INCOMPATIBLE_APPLYTYPE; END IF;
...
IF l_cnt != 1 THEN RAISE EX')||TO_CLOB('_INVALID_TARGET_VERSION; END IF;
/* TO DO
...
')||TO_CLOB('IF l_update_in_progress(l_pkgcall_id) THEN RAISE EX_UPG_DNG_IN_PROGRESS; END IF;
l_current_version := schema_version();
...
l_task VARCHAR2(20) := get_config(''TASK_VERSION'');
			...
			BEGIN
			  ...
			  log_version_end(l_version_id,''S'',NULL);
COMMIT;
l_rtn := update_task_based(l_pkgc')||TO_CLOB('all_id, p_target_version);
...
WHEN EX_NO_VERSION_HISTORY       THEN p_return_code := log_call_end(l_pkgcall_id, 02, ''F'', error_record(''Version is NULL according')||TO_CLOB(' to pdx_schema_updater tables, however, user objects were found in schema.'', NULL, NULL) );
...
WHEN OTHERS                      THEN p_return_code := log_call_end(l_pkgcall_id, 12, ''F''')||TO_CLOB(', error_record(''Unexpected Error'', SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK||CHR(10)||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE) );
END update_schema;
END;
')),
	 (TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE "ARS"."PKG_PDX_SCHEMA_UPDATER_HELPER" as
/**********************************************************************************************************************
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "ARS"."PKG_PDX_SCHEMA_UPDATER_HELPER" as
...
PRAGM')||TO_CLOB('A AUTONOMOUS_TRANSACTION;
BEGIN
IF g_debug AND g_debug_hdr_id IS NOT NULL THEN
...
END IF;
l_col := regexp_substr(l_match, ''([A-Za-z0-9_]+)\s*([A-Za-z0-9]+)\s*([(][0-9 ]*,?[0-9 ]*[)])?'',1,1')||TO_CLOB(',null,1);
...
p_coldata(l_column).tmp_type := l_t')||TO_CLOB('ype;
p_coldata(l_column).tmp_precision_length := CASE WHEN l_type = ''VARCHAR2'' THEN l_length ELSE l_precision END;
p_coldata(l_column).tmp_scale := l_scale;
EXCEPTION WHEN NO_DATA_FOUND THEN
...
-- It is possible we are restarting from a prior failure and all _ORIG columns have already been ')||TO_CLOB('resized ... but it should be an all or nothing ordeal.
...
OR p_col')||TO_CLOB('data(l_column).data_type != p_coldata(l_column).tmp_type;
...
DBMS_OUTPUT.PUT_LINE(l_column||'' C - column is already rename')||TO_CLOB('d and temp column was found''||case when p_coldata(l_column).method = ''R'' then '' [rollback]'' end);
...
p_coldata(l_column).method := ''I'';
DBMS_OUTPUT.PUT_LINE(l_column||'' I - Assumes we still have ORIG columns and simply need to rename the')||TO_CLOB('m'');
ELSE
...
END IF;
')||TO_CLOB('IF p_coldata(l_column).method = ''C'' THEN
...
l_use_ctas := TRUE;
l_sql := ''CREATE TABLE ')||TO_CLOB('''||get_temp_table_name(p_table, l_use_ctas)||'' ('';
...
FUNCTION is_oop_')||TO_CLOB('populated(p_coldata coltbl, p_table VARCHAR2, p_use_ctas BOOLEAN) RETURN BOOLEAN IS
l_column       VARCHAR2(30);
l_rtn BOOLEAN;
l_sql CLOB;
l_first_column BOOLEAN := TRUE;
...
END LOOP;
l_sql := l_sql||'' WHERE '';
l_column := p_col')||TO_CLOB('data.FIRST;
l_first_column := TRUE;
WHILE l_column IS NOT NULL LOOP
IF p_coldata(l_column).method = ''C'' THEN
l_sql := l_sql||CASE l_first_column WHEN FALSE THEN '' OR '' END||get_rename_column_name(l_column)||'' IS NOT NULL'';
...
l_column := p_coldata.FIRST;
WHILE l_column IS NOT NULL LOOP
IF p_coldata(l_column).data_type = ''VARCHAR2'' AND p_coldata(l_column).method = ''I'' AND p_coldat')||TO_CLOB('a(l_column).data_precision_length < p_coldata(l_column).tmp_precision_length THEN
IF l_first_col THEN
l_sql := ''UPDATE ''||p_table||'' SET '';
END IF;
...
END IF;
l_sql := l_sql||CASE l_first_col WHEN FALSE THEN '','' END||get_rename_column_name(l_column)||'' ''||get_data_')||TO_CLOB('type(p_coldata(l_column).data_type, p_coldata(l_column).data_precision_length, p_coldata(l_column).data_scale);
l_first_col := FALSE;
END IF;
l_column := p_coldata.NEXT(l_column);
END LOOP;
...
BEGIN
IF l_use_ctas THEN
l_sql := ''UPDATE ''||case when p_parallel > 1 THEN')||TO_CLOB(' ''/*+ enable_parallel_dml */ '' end||''(SELECT ''||case when p_parallel > 1 THEN ''/*+ parallel(''||ceil(p_parallel/2)||'') */ '' end;
l_column := p_coldata.FIRST;
l_first_column := TRUE;
WHILE l_column IS NOT NULL LOOP
IF p_coldata(l_column).method = ''T'' THEN
...
l_first_column := FALSE;
END IF;
l_column := p_coldata.NEXT(l_colu')||TO_CLOB('mn);
END LOOP;
l_sql := l_sql||'' WHERE '';
l_column := p_coldata.FIRST;
l_first_column := TRUE;
...
BEGIN
EXECUTE IMMEDIATE ''LOCK TABLE ''||l_temp_table||'' IN EXCLUSIVE M')||TO_CLOB('ODE'';
...
-- We either just created')||TO_CLOB(' out-of-place, or it already existed and all values were NULL
...
')),
	 (TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE "ARS"."PKG_PDX_SCHEMA_UPDATER_META" AS
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "ARS"."PKG_PDX_SCHEMA_UPDATER_META" AS
...
AND file_hash = NVL(pkg_pdx_schema_upda')||TO_CLOB('ter_meta.meta_hash(s.sql),0)
UNION
...
DELETE FROM pdx_schema_file_hash
WHERE file_name = (SELECT file_name FR')||TO_CLOB('OM pdx_schema_file_taskver WHERE task_version = p_task)
AND file_source = ''A'';
DELETE FROM pdx_schema_taskver_meta
...
g_task_to_value(rec.TASK_VERSION)(''A'')(rec.META_TAG)(g_task_to_value(rec.TASK_VERSION)(''A'')(rec.META_TAG).LAST):=rec.TASK_VE')||TO_CLOB('RSION_VALUE;
...
g_value_to_task(rec.TASK_VERSION_VALUE)(''B'')(rec.META_TAG)(g_value_to_tas')||TO_CLOB('k(rec.TASK_VERSION_VALUE)(''B'')(rec.META_TAG).LAST):=rec.TASK_VERSION;
END LOOP;
END;
...
RETURN NULL;
END;
FUNCTION get_metadata ( p_file_or_task VARCHAR2, p_parameter VARCHA')||TO_CLOB('R2, p_source VARCHAR2 ) RETURN VARCHAR2 IS
l_task VARCHAR2(255);
l_cnt  NUMBER;
BEGIN
...
')),
	 (TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE "ARS"."PKG_PDX_SCHEMA_UPDATER_RPT" AS
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "ARS"."PKG_PDX_SCHEMA_UPDATER_RPT" AS
...
WHERE pdx_schema_upd_pkgcall_hist_id = p_call_id
AND pdx_schema_version_histo')||TO_CLOB('ry_id = p_vers_id
ORDER BY id;
...
IF p_rec.error_message IS NOT NULL THEN
add(''          Message:  ''||SUB')||TO_CLOB('STR(REGEXP_REPLACE(p_rec.error_message,''^'',''                    '',1,0,''m''),21));
END IF;
IF p_rec.sql_error_message IS NOT NULL THEN
add(''          SqlError: ''||SUBSTR(REGEXP_REPLACE(p_rec.sql_error_message,''^'',''                    '',1,0,''m''),21));
END IF;
