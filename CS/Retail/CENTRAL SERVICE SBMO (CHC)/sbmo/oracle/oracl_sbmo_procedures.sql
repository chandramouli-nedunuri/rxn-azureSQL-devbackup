CREATE OR REPLACE procedure                                   SBMO.CUSTOM_REDEF
...
BEGIN
  ...
  v_column :=c1.column_name||' TIMESTAMP(6)';
...
select dbms_metadata.get_ddl('CONSTRAINT',c1.constraint_name,v_owner) into v_metadata from dual;
lob_replace(v_metadata,upper(v_table_name),ltrim(upper(v_rdef_table)));
lob_replace(v_metadata,upper(c1.constraint_name),ltrim(upper(v_rdef_table))||'_PK');
...
END;
  ...
  raise_application_error (-20001,v_owner||'.'||v_table_name||' : '||c6.constraint_name||'constraint did not copy sucesfully to redef table');
...
BEGIN
v_metadata := 'DROP TRIGGER '||upper(c10.object_owner)||'.'||upper(c10.object_name);
execute immediate v_metadata;
...
END custom_redef;
/
CREATE OR REPLACE PROCEDURE SBMO.SP_PURGE_RX_PRINT_CHR (p_delete_date date default null) AS
BEGIN
...
/
CREATE OR REPLACE PROCEDURE      SBMO.UPDATE_RETAIL_COST IS
...
END;
/
CREATE OR REPLACE PROCEDURE      SBMO.UPDATE_SHIPPING (p_order_number number,p_ounces float,p_tracking_number
