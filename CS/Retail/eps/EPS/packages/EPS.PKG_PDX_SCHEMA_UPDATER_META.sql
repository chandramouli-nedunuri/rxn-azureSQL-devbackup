-- ============================================================================
-- Converted: Oracle Package EPS.PKG_PDX_SCHEMA_UPDATER_META -> Azure SQL
...
CREATE OR ALTER FUNCTION [EPS].[PKG_PDX_SCHEMA_UPDATER_META_meta_version]()
RETURNS VARCHAR(20)
AS
BEGIN
    RETURN '1.01';
...
CREATE OR ALTER FUNCTION [EPS].[PKG_PDX_SCHEMA_UPDATER_META_meta_hash]
(
    @p_sql NVARCHAR(MAX)
)
RETURNS VARCHAR(255)
...
CREATE OR ALTER FUNCTION [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_taskname]
(
...
CREATE OR ALTER FUNCTION [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_filename]
(
    @p_file_or_task VARCHAR(255)
)
RETURNS VARCHAR(255)
...
CREATE OR ALTER FUNCTION [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_filetype]
(
    @p_file_or_task VARCHAR(255)
)
RETURNS VARCHAR(10)
AS
BEGIN
    DECLARE @task VARCHAR(255) = [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_taskname](@p_file_or_task);
...
CREATE OR ALTER FUNCTION [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_metadata]
(
    @p_file_or_task VARCHAR(255),
...
CREATE OR ALTER FUNCTION [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_metadata_tbl]
(
    @p_file_or_task VARCHAR(255),
    @p_parameter VARCHAR(40),
    @p_source VARCHAR(1)
...
(
    ...
    FROM [PDX_SCHEMA_TASKVER_META]
    WHERE [task_version] = [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_taskname](@p_file_or_task)
      AND [meta_tag] = UPPER(@p_parameter)
      AND [task_source] = UPPER(@p_source)
);
GO

CREATE OR ALTER FUNCTION [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_metadata_by_val]
(
    @p_file_or_task VARCHAR(255),
    @p_parameter VARCHAR(40),
    @p_source VARCHAR(1)
...
CREATE OR ALTER FUNCTION [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_metadata_tbl_by_val]
(
    @p_file_or_task VARCHAR(255),
    @p_parameter VARCHAR(40),
    @p_source VARCHAR(1)
...
(
    ...
    FROM [PDX_SCHEMA_TASKVER_META]
    WHERE [task_version_value] = [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_taskname](@p_file_or_task)
      AND [meta_tag] = UPPER(@p_parameter)
      AND [task_source] = UPPER(@p_source)
);
GO
...
CREATE OR ALTER PROCEDURE [EPS].[PKG_PDX_SCHEMA_UPDATER_META_erase_obsolete_metadata]
AS
BEGIN
    SET NOCOUNT ON;
    ...
    WHERE h.[file_source] = 'A'
      AND (s.[file_name] IS NULL OR h.[file_hash] <> ISNULL([EPS].[PKG_PDX_SCHEMA_UPDATER_META_meta_hash](CONVERT(NVARCHAR(MAX), s.[sql])), '0'));
...
CREATE OR ALTER PROCEDURE [EPS].[PKG_PDX_SCHEMA_UPDATER_META_read_metadata_filetask_map]
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE [EPS].[PKG_PDX_SCHEMA_UPDATER_META_read_metadata_parse_source]
    @p_source CHAR(1),
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN
                ...
                DECLARE @ref_task VARCHAR(255) = [EPS].[PKG_PDX_SCHEMA_UPDATER_META_get_taskname](@ref);
        ...
        VALUES (@file_name, @p_source, [EPS].[PKG_PDX_SCHEMA_UPDATER_META_meta_hash](@ddl));
...
CREATE OR ALTER FUNCTION [EPS].[PKG_PDX_SCHEMA_UPDATER_META_validate_metadata]()
RETURNS BIT
AS
BEGIN
    DECLARE @cnt INT;
    ...
    WHERE h.[file_source] = 'A'
      AND ISNULL(h.[file_hash], '0') <> ISNULL([EPS].[PKG_PDX_SCHEMA_UPDATER_META_meta_hash](CONVERT(NVARCHAR(MAX), s.[sql])), '0');
...
BEGIN
    ...
    EXEC [EPS].[PKG_PDX_SCHEMA_UPDATER_META_erase_obsolete_metadata];
    EXEC [EPS].[PKG_PDX_SCHEMA_UPDATER_META_read_metadata_filetask_map];
    EXEC [EPS].[PKG_PDX_SCHEMA_UPDATER_META_read_metadata_parse_source] @p_source = 'A', @p_task = NULL;
    EXEC [EPS].[PKG_PDX_SCHEMA_UPDATER_META_read_metadata_parse_source] @p_source = 'B', @p_task = NULL;

    EXEC sys.sp_set_session_context N'PKG_PDX_SCHEMA_UPDATER_META_CAPTURED', 1;

    IF [EPS].[PKG_PDX_SCHEMA_UPDATER_META_validate_metadata]() = 0
...
BEGIN
    ...
    EXEC [EPS].[PKG_PDX_SCHEMA_UPDATER_META_read_metadata_parse_source] @p_source = 'A', @p_task = @task;
END;
GO
