INSERT INTO ALL_OBJECTS (FILE_NAME,DDL_CLOB) VALUES
	 ('sbmo.package.cs_support.sql',TO_CLOB('
...
R')||TO_CLOB('AISE excp_no_allocatedqty;
END IF;
...
END CS_SUPPORT;
')),
	 ('sbmo.package_body.cs_support.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SBMO"."CS_SUPPORT" 
IS
excp_no_id         EXCEPTION;
excp_rowcount  EXCEPTION;
...
-- The tenant id MUST be specified in order for this to execute
-- *****************************************************************************
IF ')||TO_CLOB('p_tenantid IS NULL THEN
RAISE excp_no_tenant;
END IF;
-- *****************************************************************************
-- The value for ndc must not be null
...
WHEN OTHERS THEN
log_error (l_id, ''DBU [FORMULARY] unhandled exception encountered:'' || chr(10) || DBMS_UTILITY.FORMAT_ERROR_STACK() || chr(10) ||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE());
RAISE_APPLICATION_ERROR(-20407,''DBU [FORMULARY] unhandled exception encountered:')||TO_CLOB(''' || chr(10) || DBMS_UTILITY.FORMAT_ERROR_STACK() || chr(10) ||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE());
	END dbu_allocated_qty;
END CS_SUPPORT;
')),
	 ('sbmo.package.pkg_pdx_schema_updater.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE "SBMO"."PKG_PDX_SCHEMA_UPDATER" AS
...
END IF;
g_config_record.ALLOW')||TO_CLOB('_DOWNGRADE := NVL(get_num(get_value(''ALLOWDOWNGRADE'')),0);
...
BEGIN
RETURN pkg_pdx_schema_updater_meta.get_m')||TO_CLOB('etadata_tbl(p_file_or_task, p_parameter, CASE p_source WHEN ''U'' THEN ''B'' WHEN ''A'' THEN ''B'' WHEN ''D'' THEN ''A'' WHEN ''R'' THEN ''A'' ELSE p_source END);
...
BEGIN
BEGIN
SELECT action_code||s')||TO_CLOB('tatus_code
INTO l_status
...
VALUES ( p_version_id, p_error_record.error_message, p_error_record.sql_error_code, p_error_record.sql_er')||TO_CLOB('ror_message );
END IF;
COMMIT;
END log_version_end;
...
l_task_id := log_task_start(p_call_id, p_version_id, p_filename,')||TO_CLOB(' p_action);
END IF;
...
END IF;
UPDATE pdx_schema_updater_sql SET status_code = p_status, action_code ')||TO_CLOB('= NVL(p_action,action_code), statement_index = CASE WHEN p_status = ''S'' THEN NULL else statement_index END WHERE file_name = p_filename RETURNING action_code INTO l_action;
...
WHERE schema_owner = SYS_CONTEXT(''USERENV'',''CURRENT_SCHEMA'')
AND pdx_schema_upd_pkgcall_hist_id = p_call_id
AND pdx_schema_version_histor')||TO_CLOB('y_id = p_version_id
AND file_name = p_filename
)
WHERE id = max_id;
EXCEPTION
...
JOIN pdx_schema_taskver_me')||TO_CLOB('ta m on (m.meta_tag = ''REQUIRES'' AND task_source = ''A'' AND UPPER(m.task_version_value) = f.task_version)
...
UNION ALL SELECT ''VIEW'' object_type_n')||TO_CLOB('ame, ''SELECT'' privilege_name FROM DUAL
...
SELECT * FROM default_privs where (object_name, p_tag)')||TO_CLOB(' NOT IN (SELECT object_name, grantee FROM schema_except)
...
minus
select privilege_name privilege, object_name table_name, u.username g')||TO_CLOB('rantee
from defined_privs p
left outer join pdx_schema_mapping m on (m.mapping_type = ''USER'' and m.search = p.grantee)
left outer join all_users u on (u.username in (p.grantee, m.replace))
left outer join session_roles r on (r.role in (p.grantee, m.replace))
...
and task_source = ''A''
and ')||TO_CLOB('task_version_value = ''SYS''
...
END LOOP;
END replac')||TO_CLOB('e_tablespace;
...
l_part := regexp_replace(regexp_substr(dbms_metadata.get_ddl(l_type, l_obj, l_owner),''(PARTITION BY .*?[(]PARTITION|LOCAL .*?[(]PARTITION) .*[)]( *[^ ]* ROW MOVEMENT| *PARALLEL [^ ]*)* *$'',1,1,''n''),''PCTFREE.*? TABLES')||TO_CLOB('PACE '',''TABLESPACE '',1,0,''m'');
...
IF regexp_count(mrec.search,''^<[A-Z][A-Z0-9_]{1,29}>$'',1) != 1 OR regexp_count(mrec.replace,''^[A-Z][A-Z0-9_]{1,29}$'',1) != 1 THEN
RAISE_APPLICATION_')||TO_CLOB('ERROR(-20100,''Invalid Mapping -- PDX_SCHEMA_MAPPING_TRG should have prevented'');
END IF;
...
p_ddl_sql_tbl(l_table_index) := trim(')||TO_CLOB('l_sql);
...
l_index    NUMBER;
BEGIN
l_task_id :')||TO_CLOB('= log_sql_start(p_call_id, p_version_id, p_filename, ''A'');
...
IF NVL(p_')||TO_CLOB('action,''X'') = ''A'' THEN
...
JOIN pdx_schema_file_taskver f2 ON (UPPER(m.task_version_value) = UPPER(f2.task_vers')||TO_CLOB('ion))
...
JO')||TO_CLOB('IN file_list v ON (v.type = ''Release'' AND v.file_name = l.version_file_name)
...
l_key := TRIM(regexp_substr(l_attribcheck(l_idx),''^([^:]*):'',1,1,NULL,1))')||TO_CLOB(';
...
l_t')||TO_CLOB('ask_version VARCHAR2(20) := get_config(''TASK_VERSION'');
...
JOIN pdx_schema_file_taskver USING (task_version)
JOIN pdx_schema_updater_sql USING (file_name)
WHERE meta_tag = ''ROLLB')||TO_CLOB('ACK'' AND task_source = ''A''
AND (action_code != ''R'' OR status_code != ''S'')
...
IF new_rec.action_code = ''A'' AND new_rec.status_code = ')||TO_CLOB('''S'' THEN
BEGIN
...
IF is_same_apply_sql(rb_rec.file_na')||TO_CLOB('me) THEN
...
log_task_history (p_call_id, p_version_id, l_task, ''R'', ''F'' , error_record(''Requested task rollback, but rollback is not allowed'', NULL, NUL')||TO_CLOB('L) );
END IF;
l_task := task_list.NEXT(l_task);
END LOOP;
RETURN l_rtn;
...
pkg_pdx_schema_updater_meta.read_metadata;
	  BEGIN
SELECT CASE WHEN target_version IS NULL THEN NU')||TO_CLOB('LL ELSE current_version END INTO l_prior_version
FROM pdx_schema_version_history
...
UPDATE pdx_schema_updater_sql
			...
			END LOOP;
			INS')||TO_CLOB('ERT INTO pdx_schema_updater_manifest (version, file_name, apply_order) VALUES (manifest_rec.target_version, manifest_rec.file_name, pdx_schema_master_seq.nextval);
...
FROM pdx_schema')||TO_CLOB('_file_taskver r
...
NULL;
END;
log_sql(p_call_id, p_version_id, p_filename,''R'',''S'',NULL)')||TO_CLOB(';
END;
BEGIN
...
log_task_history ( p_call_id, l_version_id, l_task, t_act')||TO_CLOB('ion, ''W'', error_record(''No action taken: Task status is already applied'',NULL,NULL) );
...
l_value := TRIM(regexp_substr(l_attribcheck(l_idx),''^[^:]*:[ 	]*(.*)$'',1,1,NULL,1));
CASE UPPER(l_key)
WHEN NULL')||TO_CLOB(' THEN NULL;  -- Currently, there are no supported ATTRIBCHECK metadata
ELSE
...
ELSE
		              l_rtn := false;
log_task_his')||TO_CLOB('tory(p_call_id, l_version_id,l_task, t_action, ''F'', error_record(''Requested task rollback, but rollback is done by another task'',NULL,NULL) );
END IF;
ELSE
l_rtn := false;
...
where compare_versions(c_source_version,c_target_version) < 0
and compare_versions(lead_version,c_source_version) > 0
and compare_versions(lead_version,c_target_ve')||TO_CLOB('rsion) <= 0
...
l_result := version_updater(p_call_id, manifest_rec.target_version, l_current_version, manifest_rec.file_name, ''D'');')||TO_CLOB('
END IF;
CLOSE manifest_cur;
...
EX_NOTHING_TO_DO               EXCEPTION')||TO_CLOB(';
...
END IF;
IF g')||TO_CLOB('et_config(''TASK_VERSION'') = p_current_version AND get_config(''APPLY_TYPE'') != ''TASK'' THEN RAISE EX_INCOMPATIBLE_APPLYTYPE; END IF;
...
IF l_cnt != 1 THEN RAISE ')||TO_CLOB('EX_INVALID_TARGET_VERSION; END IF;
/* TO DO
...
l_task VARCHAR2(20) := get_config(''TASK_VERSION'');
			...
			BEGIN
			  ...
			  log_version_end(l_version_id,''S'',NULL);
COMMIT;
l_rtn := update_task_based(l_pk')||TO_CLOB('gcall_id, p_target_version);
...
WHEN EX_NO_VERSION_HISTORY       THEN p_return_code := log_call_end(l_pkgcall_id, 02, ''F'', error_record(''Version is NULL accordi')||TO_CLOB('ng to pdx_schema_updater tables, however, user objects were found in schema.'', NULL, NULL) );
...
WHEN OTHERS                      THEN p_return_code := log_call_end(l_pkgcall_id, 12, ''')||TO_CLOB('F'', error_record(''Unexpected Error'', SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK||CHR(10)||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE) );
END update_schema;
END;
')),
	 ('sbmo.package_body.pkg_pdx_schema_updater.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SBMO"."PKG_PDX_SCHEMA_UPDATER" AS
g_version CONSTANT VARCHAR2(4) := ''1.03'';
TYPE typ_error_record IS RECORD (
ERROR_MESSAGE            VARCHAR2(200),
...
function get_num ( p_val varchar2 ) return number is
invalid_number exception;
pragma exception_init(')||TO_CLOB('invalid_number,-6502);
l_rtn number;
begin
IF get_yn(p_val) = ''Y'' then l_rtn := -1;
ELSE
...
WHEN ''MANAGE_SYNONYMS_FOR''  THEN RETURN g_config_record.MANAGE_SYNONYMS_FOR;
WHEN ''AUTO_UNDO''            THEN RETURN g_config_record.AUTO_UNDO;
ELSE RAISE_APPLICATION_ERROR(-20100,''Invalid Key requested in')||TO_CLOB(' get_config'');
END CASE;
END get_config;
FUNCTION compare_versions (p_version1 VARCHAR2, p_version2 VARCHAR2) RETURN NUMBER IS
l_rtn NUMBER;
...
RETURN pkg_pdx_schema_updater_meta.get_metadata_tbl_by_val(p_file_or_task, p_parameter, CASE p_source WHEN ''U'' THEN ''B'' WHEN ''A'' THEN ''B'' WHEN ''D'' THEN ''A'' WHEN ''R'' THEN ''A'' ELSE p_source END);
END get_metadata_tbl_by_val;
FUNCTION get_tasktype(p_filename VARCH')||TO_CLOB('AR2, p_action VARCHAR2) RETURN VARCHAR2 IS
BEGIN
RETURN NVL(UPPER(get_metadata(p_filename,''TASKTYPE'',p_action)),UPPER(get_config(''APPLICATION_PREFIX'')));
END;
	  FUNCTION get_sql ( p_fname VARCHAR2, p_action VARCHAR2 ) RETURN CLOB IS
...
IF NOT is_error_record_empty(p_error_record) THEN
INSERT INTO PDX_SCHEMA_UPD_PKGCALL_ERR_LOG ( id, error_message, sql_error_code, sql_error_message )
VALUES ( p_call_id,')||TO_CLOB(' p_error_record.error_message, p_error_record.sql_error_code, p_error_record.sql_error_message );
END IF;
COMMIT;
RETURN p_rc;
END log_call_end;
...
PROCEDURE log_task_end ( p_task_id NUMBER, p_status VARCHAR2, p_error_record typ_error_record, p_action VARCHAR2 := NULL, p_sql_text CLOB := NULL, p_index NUMBER := NULL ) IS
BEGIN
UPDATE pdx_schema_task_history SET status_code = p_status, action_code = NVL(p_action, action_code), end_date = SYSTIMESTAMP WHERE id')||TO_CLOB(' = p_task_id;
IF NOT is_error_record_empty(p_error_record) THEN
INSERT INTO PDX_SCHEMA_TASK_ERROR_LOG ( id, error_message, sql_error_code, sql_error_message, sql_text, statement_index )
VALUES ( p_task_id, p_error_record.error_message, p_error_record.sql_error_code, p_error_record.sql_error_message, p_sql_text, p_index );
END IF;
...
pkg_pdx_schema_updater_meta.refresh_metadata_apply(p_filename);
IF pkg_pdx_schema_updater_meta.get_filetype(p_filename) = ''R'' AND p_action = ''U'' THEN
	    MERGE I')||TO_CLOB('NTO pdx_schema_updater_manifest tgt USING (SELECT target_version version FROM pdx_schema_version_history WHERE id = p_version_id) src ON (src.version = tgt.version)
		  WHEN NOT MATCHED THEN INSERT (file_name, version, apply_order) VALUES (p_filename, src.version, PDX_SCHEMA_MASTER_SEQ.NEXTVAL)
		  WHEN MATCHED THEN UPDATE SET apply_order = PDX_SCHEMA_MASTER_SEQ.NEXTVAL;
END IF;
COMMIT;
...
PROCEDURE log_sys_sql_end ( p_call_id NUMBER, p_version_id NUMBER, p_task_id NUMBER, p_filename VARCHAR2 ) IS
l_status VARCHAR2(1);
l_error_r')||TO_CLOB('ecord typ_error_record;
l_action   VARCHAR2(1);
l_sql_text VARCHAR2(4000);
l_index    NUMBER(38);
BEGIN
...
BEGIN
UPDATE schema_updater_sql
SET s')||TO_CLOB('ql = regexp_replace(sql,CHR(13)||CHR(10),CHR(10))
WHERE instr(sql,CHR(13)||CHR(10)) > 0;
COMMIT;
END;
PROCEDURE purge_schema_error_logs (p_until_date  DATE) IS
...
FROM pdx_schema_file_taskver f
JOIN pdx_schema_taskver_meta m on (m.meta_tag IN (''REQUIRES'',''ROLLBACK'') AND m.task_source = ''B'' AND m.task_version = f.task_version)
WHERE (f.file_name = p_fname OR f.ta')||TO_CLOB('sk_version = UPPER(p_fname))
AND UPPER(m.task_version_value) NOT IN (''YES'',''NO'',''WRAPPED'',''TRUE'',''FALSE'')  -- We assume any other value is a task reference
MINUS
SELECT UPPER(REGEXP_SUBSTR(file_name,''^''||l_app_prefix||''_(.*?)(_ddl\.sql)$'',1,1,''i'',1))
FROM pdx_schema_updater_sql
...
SELECT UPPER(object_type) object_type, UPPER(object_name) object_name, UPPER(privilege) privilege, UPPER(grantee) grantee FROM pdx_schema_priv_exception UNION
SELECT ''TABLE'',''PDX_SCHEMA_VERSION_HISTORY''    ,''SELECT'',''<''||''APP_USER''||''>'' FROM DUAL UNION
SELECT ''TABLE'',''PDX_SCHEMA_UPD_PKGCALL_HIST''   ,''SELECT'',''<''||''APP_USER''||''>'' FROM DUAL UNIO')||TO_CLOB('N
SELECT ''TABLE'',''PDX_SCHEMA_UPD_PKGCALL_ERR_LOG'',''SELECT'',''<''||''APP_USER''||''>'' FROM DUAL UNION
SELECT ''TABLE'',''PDX_SCHEMA_UPDATER_SQL''        ,''SELECT'',''<''||''APP_USER''||''>'' FROM DUAL UNION
SELECT ''TABLE'',''PDX_SCHEMA_UPDATER_MANIFEST''   ,''SELECT'',''<''||''APP_USER''||''>'' FROM DUAL UNION
SELECT ''TABLE'',''PDX_SCHEMA_TASK_HISTORY''       ,''SELECT'',''<''||''APP_USER''||''>'' FROM DUAL UNION
...
SELECT ''GRANT'' action,privilege,table_name,grantee
FROM (select privilege_name privilege, object_name table_name, u.username grantee')||TO_CLOB('
from defined_privs p
left outer join pdx_schema_mapping m on (m.mapping_type = ''USER'' and m.search = p.grantee)
left outer join all_users u on (u.username in (p.grantee, m.replace))
left outer join session_roles r on (r.role in (p.grantee, m.replace))
...
FOR urec IN (SELECT search, replace FROM pdx_schema_mapping WHERE mapping_type IN (''USER'',''ROLE'')) LOOP
IF urec.replace IS NOT NULL THEN
FOR rec IN cur_generate_priv_dd')||TO_CLOB('l(urec.search,urec.replace) LOOP
-- We need to lock down the Schema Updater tables in case they gave ALL to some other user.
IF urec.search!=''<''||''APP_USER''||''>'' AND (   (rec.TABLE_NAME LIKE ''PDX\_SCHEMA\_%\_SEQ'' ESCAPE ''\'')
OR (rec.TABLE_NAME LIKE ''PKG_PDX_SCHEMA%'')
OR (rec.TABLE_NAME LIKE ''PDX\_SCHEMA\_%'' ESCAPE ''\'' AND rec.PRIVILEGE != ''SELECT'')
...
FROM user_objects
WHERE object_name NOT LIKE ''PDX_SCHEMA%''
AND object_name NOT LIKE ''SCHEMA_UP')||TO_CLOB('DATER%''
AND object_name NOT LIKE ''PKG_PDX_SCHEMA_UPDATER%''
AND object_name NOT LIKE ''VW_SCHEMA_UPDATER%''
AND object_type IN (''TABLE'',''VIEW'',''PROCEDURE'',''FUNCTION'',''PACKAGE'',''SEQUENCE'')
				  ) LOOP
...
l_obj := UPPER(regexp_substr(l_meta,''<partition:([^:>]*):([^>.]*)[.]([^>]*)>'',1,1,''im'',3));
ELSE
l_owner := SYS_CONTEXT(''USERENV'',''CURRENT')||TO_CLOB('_SCHEMA'');
l_obj := UPPER(regexp_substr(l_meta,''<partition:([^:>]*):([^>]*)>'',1,1,''im'',2));
END IF;
SELECT COUNT(*) INTO l_cnt FROM all_objects WHERE owner = l_owner and object_name = l_obj and object_type = l_type;
IF l_cnt = 1 THEN
...
l_table varchar2(32);
l_action varchar2(10);
l_own varchar2(32) := ''''''''||SYS_CONTEXT(''USERENV'',''CURRENT_SCHEMA'')||''''')||TO_CLOB(''''';
begin
l_line := regexp_substr(p_ddl_sql,''^ *(--\*)? *<sbmo_meijer:[^>]*> *;? *$'',1,1,''im'');
while l_line IS NOT NULL LOOP
l_table := ''''''''||regexp_substr(l_line,''<sbmo_meijer:([^:>]*)[:>]'',1,1,''im'',1)||'''''''';
...
IF INSTR(UPPER(NVL(l_input_line, ''Z'')), ''CREATE OR REPLACE'') <> 0 AND
(INSTR(UPPER(NVL(l_input_line, ''Z'')), '' FUNCTION '') <> 0       OR
INSTR(UPPER(N')||TO_CLOB('VL(l_input_line, ''Z'')), '' PACKAGE '') <> 0        OR
INSTR(UPPER(NVL(l_input_line, ''Z'')), '' PROCEDURE '') <> 0      OR
INSTR(UPPER(NVL(l_input_line, ''Z'')), '' TRIGGER '') <> 0        OR
INSTR(UPPER(NVL(l_input_line, ''Z'')), '' TYPE '') <> 0)          THEN
l_terminator := ''/'';
...
l_task_id  NUMBER;
BEGIN
l_rb_found := REGEXP_INSTR(l_sql,''^\s*')||TO_CLOB('--\*\s*\S'',1,1,0,''m'') > 0;
--If we don''t have the TASKTYPE metadata, then assumed to be sql-based release file executed as tasK
IF pkg_pdx_schema_updater_meta.get_filetype(p_filename) = ''T'' THEN
l_rb_ok := UPPER(NVL(get_metadata(p_filename,''ROLLBACK'',p_action),''YES'')) IN (''TRUE'',''YES'');
ELSE
...
EXCEPTION
WHEN OTHERS THEN
log_task_history( p_call_id, p_version_id, p_filename, NVL(p_action,''X''), ''F'', error_record(NU')||TO_CLOB('LL, SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK||CHR(10)||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE) );
RETURN FALSE;
END run_sql;
FUNCTION is_deprecated ( p_filename pdx_schema_updater_sql.file_name%TYPE,
p_task_list task_tbl
...
l_task := task_proc(task_proc_ind);
END IF;
FOR ctr IN ')||TO_CLOB('1..task_list.COUNT LOOP
IF is_deprecated(l_task,task_list) THEN
IF NVL(get_status(l_task),''RS'') IN (''RS'',''RD'') THEN
log_sql(p_call_id, p_version_id, l_task, ''A'', ''D'',error_record(''Task has been deprecated and will not be applied'',NULL,NULL) );
END IF;
...
SELECT t.f')||TO_CLOB('ile_name, v.info release_version
FROM task_list l
JOIN file_list t ON (t.type = ''Task'' AND l.task = t.info)
JOIN file_list v ON (v.type = ''Release'' AND v.file_name = l.version_file_name)
JOIN schema_updater_manifest m ON (m.file_name = l.version_file_name)
...
FROM pdx_schema_updater_sql s
JOIN pdx_schema_file_taskver f ON (f.file_name = s.file_name)
JOIN pdx_schema_taskver_meta m ON (m.task_version = f.task_version AND m.meta_tag IN (''REQUIRES'',''ROLLBACK'') AND ')||TO_CLOB('m.task_source = ''A'')
WHERE (action_code != ''R'' OR status_code != ''S'')
AND version = l_task_version
AND upper(m.task_version_value) = pkg_pdx_schema_updater_meta.get_taskname(p_fname);
l_tname    VARCHAR2(50) := UPPER(REGEXP_SUBSTR(p_fname,''^(''||get_config(''APPLICATION_PREFIX'')||''_)(.*?)(_ddl\.sql)$'',1,1,''im'',2));
...
PROCEDURE log_rb_sql_history (p_call_id pdx_schema_upd_pkgcall_hist.id%TYPE,
p_version_id pdx_schema_version_history.id%TYPE,
p_filename pdx_schema_task_history.file_name%TYPE) ')||TO_CLOB('IS
l_id pdx_schema_task_history.id%TYPE;
BEGIN
-- ROLLBACK;TASK mistakenly logged as rollback is not allowed
BEGIN
...
IF UPPER(NVL(get_metadata(l_task,''ROLLBACK'',''R''),''YES'')) IN (''YES'',''NO'',''TRUE'',''FALSE'',''NO'',''WRAPPED'') THEN
l_processed := true;
l_result := run_sql(p_call_id, p_ver')||TO_CLOB('sion_id, l_task, ''R'');
l_rtn := l_rtn AND l_result;
task_list.DELETE(l_task);
task_proc.DELETE(task_proc_ind);
IF l_rtn THEN
...
END update_task_based;
PROCEDURE convert_to_task IS
l_current_version       pdx_schema_version_history.')||TO_CLOB('target_version%TYPE;
l_call_id               pdx_schema_upd_pkgcall_hist.id%TYPE;
l_version_id            NUMBER;
l_taskversion           VARCHAR2(20) := get_config(''TASK_VERSION'');
l_dummy                 NUMBER;
...
and compare_versions(lead_version,c_source_version) > 0
and compare_versions(lead_version,c_target_version) <= 0
union all ')||TO_CLOB('select version, file_name, apply_order from schema_updater_manifest WHERE c_source_version IS NULL
			           and compare_versions(version,c_target_version)<=0
		 ORDER BY apply_order;
manifest_rec manifest_cur%ROWTYPE;
	  CURSOR sql_cur (c_release_file VARCHAR2, c_validate_update_flag VARCHAR2) IS
...
l_version_id := log_version_start(l_call_id, l_taskversion, NVL(p_version,l_prior_version));
IF p_version IS NOT NULL THEN
IF compar')||TO_CLOB('e_versions(l_prior_version,p_version) > 0 THEN
		    RAISE_APPLICATION_ERROR(-20100,''You cannot convert to a release prior to the point of conversion.  Earliest allowed release is ''||l_prior_version);
		  END IF;
		  OPEN manifest_cur(l_prior_version,p_version);
		  FETCH manifest_cur INTO manifest_rec;
...
UPDATE pdx_schema_config set value = ''RELEASE'' WHERE KEY = ''APPLYTYPE'';
		...
		ELSE
		  log_version_end(l_version_id, ''F'', NULL);
		  l_dummy := log_cal')||TO_CLOB('l_end(l_call_id, 11, ''F'', NULL);
END IF;
END IF;
END;
FUNCTION version_updater (p_call_id           pdx_schema_upd_pkgcall_hist.id%TYPE,
...
FROM pdx_schema_taskver_meta
JOIN pdx_schema_file_taskver USING (task_version)
JOIN pdx_schema_updater_sql ')||TO_CLOB('USING (file_name)
WHERE meta_tag = ''ROLLBACK'' AND task_source = ''A''
AND (action_code != ''R'' OR status_code != ''S'')
AND upper(task_version_value) = upper(pkg_pdx_schema_updater_meta.get_taskname(c_task));
PROCEDURE log_rb_sql_history (p_call_id pdx_schema_upd_pkgcall_hist.id%TYPE,
...
RETURN FALSE;
END IF;
l_dummy_id := log_sql_start(p_call_id, l_version_id, p_filename, p')||TO_CLOB('_action);
task_list.DELETE;
FOR sql_task_rec IN sql_task_csr (p_filename, p_action) LOOP
l_task := sql_task_rec.file_name;
BEGIN
...
select target_version, file_name
FROM (SELECT target_version, file_name, apply_order
FROM (select')||TO_CLOB(' CASE WHEN c_target_version IS NULL OR compare_versions(lag_version,c_target_version) > 0 THEN lag_version ELSE c_target_version END target_version,
file_name,
m1.apply_order
from (select file_name,
apply_order,
...
SELECT current_version,
			       target_version
INT')||TO_CLOB('O l_cversion, l_tversion
FROM pdx_schema_version_history
WHERE id = (SELECT MAX(id)
FROM pdx_schema_version_history
WHERE (   target_version = pkg_pdx_schema_updater.schema_version
...
l_action_code := ''D'';
END IF;
l_current_version := pkg_pdx_schema_updater.schema_versi')||TO_CLOB('on;
OPEN manifest_cur(pkg_pdx_schema_updater.schema_version, p_target_version);
FETCH manifest_cur INTO manifest_rec;
WHILE manifest_cur%FOUND AND l_result LOOP
l_result := version_updater(p_call_id, l_current_version, manifest_rec.target_version, manifest_rec.file_name, l_action_code);
...
BEGIN
IF p_current_version IS NULL THEN
SELECT COUNT(*) INTO l_non_pdx_object_count FROM user_objects WHERE object_name NOT LIKE ''PDX_SCHEMA%'' AND object_name NOT LIKE ''SCHEMA_UPDATE')||TO_CLOB('R%'' AND object_name NOT LIKE ''PKG_PDX_SCHEMA_UPDATER%'' AND object_name NOT LIKE ''VW_SCHEMA_UPDATER%'' AND object_type IN (''TABLE'',''VIEW'',''PROCEDURE'',''FUNCTION'',''PACKAGE'',''SEQUENCE'');
		    IF l_non_pdx_object_count != 0 THEN
SELECT COUNT(*) INTO l_cnt FROM (SELECT file_name FROM pdx_schema_updater_sql WHERE version != p_target_version AND (action_code NOT IN (''R'',''D'') OR status_code NOT IN (''S'',''X'',''D'')) UNION ALL SELECT file_name FROM pdx_schema_updater_manifest WHERE version != p_target_version);
BEGIN SELECT value INTO l_value FROM pdx_schema_config WHERE key = ''PREEXISTINGSCHEMA'' and source = ''A''; EXCEPTION WHEN NO_DATA_FOUND THEN l_value := NULL; END;
IF l_cnt > 0 OR NVL(l_value,''Y'') = ''N'' THEN RAISE EX_NO_VERSION_HISTORY; END IF;
...
IF p_target_version IS NOT NULL THEN
SELECT COUNT(*) INTO l_cnt FROM pdx_schema_updater_manifest m JOIN pdx_schema_updater_sql s USING (file_name) WHERE m.version = p_target_version;
-- IF p_target_version IS in pdx_schema_updater_manifest then we are downgrading an')||TO_CLOB('d don''t require schema_updater_manifest
IF l_cnt != 1 THEN
SELECT COUNT(*) INTO l_cnt FROM schema_updater_manifest m JOIN schema_updater_sql s USING (file_name) WHERE m.version = p_current_version;
IF l_cnt != 1 THEN RAISE EX_INVALID_CURRENT_VERSION; END IF;
END IF;
...
BEGIN SELECT file_name INTO b_file FROM schema_updater_manifest WHERE apply_order = (SELECT MAX(m.apply_order) FROM schema_updater_manifest m JOIN pdx_schema_updater_manifest USING (file_name)); EXCEPTION WHEN NO_DATA_FOUND THEN NULL; END;
-- If l_cnt = 2 then we are downgrading so order doesn''t matter
SELECT COUNT(*) + CASE WHEN p_current_version ')||TO_CLOB('IS NULL THEN 1 ELSE 0 END + CASE WHEN p_target_version IS NULL THEN 1 ELSE 0 END + CASE WHEN p_current_version = p_target_version THEN 1 ELSE 0 END INTO l_cnt FROM pdx_schema_updater_manifest WHERE version IN (p_current_version, p_target_version);
IF a_file IS NOT NULL AND (b_file IS NULL OR b_file != a_file) AND l_cnt != 2 THEN RAISE EX_CORRUPT_MANIFEST; END IF;
END l_validate_manifest;
PROCEDURE l_validate_action ( p_current_version VARCHAR2, p_target_version VARCHAR2 ) IS
	      l_rb NUMBER := get_config(''ALLOW_DOWNGRADE'');
...
l_process_id := log_process_start(l_pkgcall_id, NULL, ''Reading metadata''); pkg_pdx_schema_updater_meta.read_metadata; log_process_end(l_process_id);
IF get_config(''APPLY_TYPE'') = ''TASK'' THEN
l_rtn := update_task_ba')||TO_CLOB('sed(l_pkgcall_id, p_target_version);
ELSE
l_rtn := update_release_based(l_pkgcall_id, l_current_version, p_target_version);
IF NOT l_rtn AND get_config(''AUTO_UNDO'') = ''Y'' THEN
l_undo_rtn := update_release_based(l_pkgcall_id, schema_version(), l_current_version);
...
reset_schema;
	  ...
	  END IF;
IF get_config(''MANAGE_PRIVS'') = ''Y'' THEN l_process_id := log_process_start(l_pkgcall_id, NULL, ''Granting Privileges''); grant_privs_on_own_objs; log_process_end(l_process_id); END IF;
IF ge')||TO_CLOB('t_config(''MANAGE_SYNONYMS'') = ''Y'' THEN l_process_id := log_process_start(l_pkgcall_id, NULL, ''Creating Synonyms''); create_synonyms_on_own_objs; log_process_end(l_process_id); END IF;
IF NOT l_rtn THEN
IF get_config(''APPLY_TYPE'') = ''TASK'' THEN RAISE EX_TASK_FAILED;
ELSE                                      RAISE EX_RELEASE_FAILED;
END IF;
...
WHEN EX_INCOMPATIBLE_APPLYTYPE   THEN p_return_code := log_call_end(l_pkgcall_id, 04, ''F'', error_record(''Incompatible ApplyType [''||get_config(''APPLY_TYPE'')||'']: Current Version [''||l_current_version||''] Target Version [''||p_target_version||''].'', NULL, NULL) );
WHEN EX_PDX_TABLES_CORRUPT       THEN p_return_code := log_call_end(l_pkgcall_id, 05, ''F'', error_record(''Current Version [''||l_current_version||''] is missing from pdx_schema_updater_manifest/sql.'', NULL, NULL) );
WHEN EX_INVALID_CURRENT_VERSION  THEN p_return_code := log_call_end(l_pkgcall_id, 06, ''F')||TO_CLOB(''', error_record(''Current Version [''||l_current_version||''] is missing from schema_updater_manifest/sql.'', NULL, NULL) );
WHEN EX_DOWNGRADE_NOT_ALLOWED    THEN p_return_code := log_call_end(l_pkgcall_id, 07, ''F'', error_record(''Downgrade has been requested, however, configuration does not allow downgrade.  Please contact DBA for assistance.'', NULL, NULL) );
WHEN EX_INVALID_CONFIG_KEY       THEN p_return_code := log_call_end(l_pkgcall_id, 08, ''F'', error_record(''Oops.  Requested invalid Configuration Key -- Schema Updater code issue.'', NULL, NULL) );
WHEN EX_CORRUPT_MANIFEST         THEN p_return_code := log_call_end(l_pkgcall_id, 09, ''F'', error_record(''Conflicting information (Version apply order) was found in pdx_schema_updater_manifest and schema_updater_manifest.'', NULL, NULL) );
WHEN EX_RELEASE_FAILED           THEN p_return_code := log_call_end(l_pkgcall_id, 10, ''F'', NULL);
...
END;
')),
	 ('sbmo.package.pkg_pdx_schema_updater_helper.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE "SBMO"."PKG_PDX_SCHEMA_UPDATER_HELPER" as
/**********************************************************************************************************************
...
***********************************************************************************************************************
* Drops unused columns from table_name when provided')||TO_CLOB(', otherwise drops unused columns from all tables
***********************************************************************************************************************/
...
PRA')||TO_CLOB('GMA AUTONOMOUS_TRANSACTION;
BEGIN
IF g_debug AND g_debug_hdr_id IS NOT NULL THEN
...
END IF;
l_col := regexp_substr(l_match, ''([A-Za-z0-9_]+)\s*([A-Za-z0-9]+)\s*([(][0-9 ]*,?[0-9 ]*[)])?'',1')||TO_CLOB(',1,null,1);
...
p_coldata(l_column).tmp_type := l')||TO_CLOB('_type;
p_coldata(l_column).tmp_precision_length := CASE WHEN l_type = ''VARCHAR2'' THEN l_length ELSE l_precision END;
p_coldata(l_column).tmp_scale := l_scale;
EXCEPTION WHEN NO_DATA_FOUND THEN
...
-- It is possible we are restarting from a prior failure and all _ORIG columns have already bee')||TO_CLOB('n resized ... but it should be an all or nothing ordeal.
...
OR p_c')||TO_CLOB('oldata(l_column).data_type != p_coldata(l_column).tmp_type;
...
DBMS_OUTPUT.PUT_LINE(l_column||'' C - column is already rena')||TO_CLOB('med and temp column was found''||case when p_coldata(l_column).method = ''R'' then '' [rollback]'' end);
...
p_coldata(l_column).method := ''I'';
DBMS_OUTPUT.PUT_LINE(l_column||'' I - Assumes we still have ORIG columns and simply need to rename t')||TO_CLOB('hem'');
ELSE
...
l_use_ctas := TRUE;
l_sql := ''CREATE TABL')||TO_CLOB('E ''||get_temp_table_name(p_table, l_use_ctas)||'' ('';
...
FUNCTION is_oo')||TO_CLOB('p_populated(p_coldata coltbl, p_table VARCHAR2, p_use_ctas BOOLEAN) RETURN BOOLEAN IS
l_column       VARCHAR2(30);
l_rtn BOOLEAN;
l_sql CLOB;
l_first_column BOOLEAN := TRUE;
...
END LOOP;
l_sql := l_sql||'' WHERE '';
l_column := p_c')||TO_CLOB('oldata.FIRST;
l_first_column := TRUE;
WHILE l_column IS NOT NULL LOOP
IF p_coldata(l_column).method = ''C'' THEN
l_sql := l_sql||CASE l_first_column WHEN FALSE THEN '' OR '' END||get_rename_column_name(l_column)||'' IS NOT NULL'';
...
l_column := p_coldata.FIRST;
WHILE l_column IS NOT NULL LOOP
IF p_coldata(l_column).data_type = ''VARCHAR2'' AND p_coldata(l_column).method = ''I'' AND p_cold')||TO_CLOB('ata(l_column).data_precision_length < p_coldata(l_column).tmp_precision_length THEN
IF l_first_col THEN
l_sql := ''UPDATE ''||p_table||'' SET '';
END IF;
...
END IF;
l_sql := l_sql||CASE l_first_col WHEN FALSE THEN '','' END||get_rename_column_name(l_column)||'' ''||get_dat')||TO_CLOB('a_type(p_coldata(l_column).data_type, p_coldata(l_column).data_precision_length, p_coldata(l_column).data_scale);
l_first_col := FALSE;
END IF;
l_column := p_coldata.NEXT(l_column);
END LOOP;
...
BEGIN
IF l_use_ctas THEN
l_sql := ''UPDATE ''||case when p_parallel > 1 TH')||TO_CLOB('EN ''/*+ enable_parallel_dml */ '' end||''(SELECT ''||case when p_parallel > 1 THEN ''/*+ parallel(''||ceil(p_parallel/2)||'') */ '' end;
l_column := p_coldata.FIRST;
l_first_column := TRUE;
WHILE l_column IS NOT NULL LOOP
IF p_coldata(l_column).method = ''T'' THEN
...
l_first_column := FALSE;
END IF;
l_column := p_coldata.NEXT(l_co')||TO_CLOB('lumn);
END LOOP;
l_sql := l_sql||'' WHERE '';
l_column := p_coldata.FIRST;
l_first_column := TRUE;
...
BEGIN
EXECUTE IMMEDIATE ''LOCK TABLE ''||l_temp_table||'' IN EXCLUSIVE')||TO_CLOB(' MODE'';
...
BEGIN
-- tmp_renamed          ')||TO_CLOB('  VARCHAR2(1),        -- N (No), Y (Yes)
-- new_renamed            VARCHAR2(1),        -- N (No), C (Column), T (Table)
-- method                 VARCHAR2(1)         -- N (None), I (In Place), C (Out of Place - Column), T (Out of Place - Table)
l_column := p_coldata.FIRST;
WHILE l_column IS NOT NULL LOOP
...
-- We either just creat')||TO_CLOB('ed out-of-place, or it already existed and all values were NULL
...
')),
	 ('sbmo.package_body.pkg_pdx_schema_updater_helper.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SBMO"."PKG_PDX_SCHEMA_UPDATER_HELPER" as
DTL_EXCEPTION EXCEPTION;
g_is_release BOOLEAN := NULL;
g_debug_hdr_id NUMBER(38);
...
PRAGMA AUTONOMOUS_TRANSACTION;
BEGIN
IF g_debug AND g_debug_dtl_id IS NOT N')||TO_CLOB('ULL THEN
UPDATE pdx_schema_upd_helper_debug_d SET end_timestamp = SYSTIMESTAMP WHERE id = g_debug_dtl_id;
g_debug_dtl_id := NULL;
COMMIT;
END IF;
...
RETURN CASE WHEN p_use_ctas THEN SUBSTR(UPPER(p_table),1,26)||''_TMP'' ELSE UPPER(p_table) END;
END get_temp_table_name;
FUNCTION get_temp_column_name (p_colum')||TO_CLOB('n VARCHAR2, p_use_ctas BOOLEAN) RETURN VARCHAR2 DETERMINISTIC IS
BEGIN
RETURN CASE WHEN p_use_ctas THEN UPPER(p_column) ELSE SUBSTR(UPPER(p_column),1,25)||''_TMP'' END;
END get_temp_column_name;
FUNCTION get_data_type (p_type VARCHAR2, p_prec_len NUMBER, p_scale NUMBER := NULL) RETURN VARCHAR2 DETERMINISTIC IS
...
l_colrow.data_type := l_type;
l_colrow.data_precision_length := regexp_substr(l_tmp, ''[(]([0-9]*),?([0-9]*)?[)]'',1,1,null,1);
l_colrow.data_scale := regexp_substr(l_tmp, ''[(]([0-9]*),?(')||TO_CLOB('[0-9]*)?[)]'',1,1,null,2);
p_coldata(l_col) := l_colrow;
l_change_list := regexp_replace(l_change_list, REPLACE(REPLACE(l_match,''('',''[(]''),'')'',''[)]''), null, 1, 1, ''i'');
END LOOP;
IF l_change_list IS NOT NULL THEN
...
DECLARE l_temp_column VARCHAR2(30) := get_temp_column_name(l_column,false);
BEGIN
SELECT data_type, data_length, data_prec')||TO_CLOB('ision, data_scale INTO l_type, l_length, l_precision, l_scale FROM user_tab_columns WHERE table_name = UPPER(p_table) and column_name = l_temp_column;
IF p_use_ctas THEN
RAISE_APPLICATION_ERROR(-20001,''Requested use of CTAS, but temporary column ''||l_temp_column||'' exists on table.'');
END IF;
p_coldata(l_column).new_renamed := ''C'';
...
-- But we could have a mixure of In place and Out of place modifications if multiple columns are provided
l_decreasing := (    (    p_coldata(l_column).data_type = ''NUMBER'' AND p_coldata(l_column).tmp_type = ''NUMBER''
AND (   NVL(p_coldata(l_column).data_precision_length,0)-NVL(p_coldata(l_column).data_scale,0) < NVL(p_coldata(l_column).tmp_precision_length,0)-NVL(p_coldata(l_column).')||TO_CLOB('tmp_scale,0)
OR NVL(p_coldata(l_column).data_precision_length,0) < NVL(p_coldata(l_column).tmp_precision_length,0)
OR NVL(p_coldata(l_column).data_scale,0) < NVL(p_coldata(l_column).tmp_scale,0)
)
)
...
IF p_coldata(l_column).new_renamed != ''N'' THEN
-- This is an error because if done by package then original column (tmp_renamed) would be renamed
RAI')||TO_CLOB('SE_APPLICATION_ERROR(-20002,''It appears that column ''||l_column||'' is already of the requested size, but a temporary column exists.'');
END IF;
p_coldata(l_column).method := ''N'';
DBMS_OUTPUT.PUT_LINE(l_column||'' N - Not renamed and is neither increasing, decreasing, nor changing datatype'');
ELSIF p_coldata(l_column).tmp_renamed = ''Y'' THEN
...
END IF;
p_coldata(l_column).method := ''T'';
DBMS_OUTPUT.PUT_LINE(l_column||'' T - column is already renamed and temp table was found''||case when p_coldata(l_column).met')||TO_CLOB('hod = ''R'' then '' [rollback]'' end);
END IF;
END IF;
IF p_coldata(l_column).method IS NULL THEN
-- Do not need to validate new_renamed becuase if it was set, then method is non-null
...
l_tc_action   VARCHAR2(1);
BEGIN
-- tmp_renamed c')||TO_CLOB('ould be a mixture as each rename takes place individually.
-- All new_renamed should be the same value.  All N if tmp_renamed is N, otherwise matching the method
-- tmp_renamed            VARCHAR2(1),        -- N (No), Y (Yes)
-- new_renamed            VARCHAR2(1),        -- N (No), C (Column), T (Table)
-- method                 VARCHAR2(1)         -- N (None), I (In Place), C (Out of Place - Column), T (Out of Place - Table)
...
END IF;
ELSIF p_coldata(l_column).method = ''T'' THEN
IF p_coldata(l_column).new_renamed NOT IN (''N'',''T'') THEN')||TO_CLOB('
RAISE_APPLICATION_ERROR(-20003,''Found ''||l_column||'' In temporary column, but was expecting temporary table.'');
END IF;
IF NOT p_use_ctas THEN
RAISE_APPLICATION_ERROR(-20003,l_column||'' logic would use Temp Columns, but CTAS was requested.'');
...
-- What if we are changing the datatype on a Primary Key field?
-- Assumption: It will be increase-only which means we will be in-place, not ctas, for those columns
FOR pk IN (SELECT cc.column_name, tc.data_type, tc.data_length, tc.data_precision, tc.data_scale FROM user_constraints c join user_cons_columns cc on (cc.constraint_name = c.constraint_name) join user_tab_columns tc on (tc.ta')||TO_CLOB('ble_name = c.table_name and tc.column_name = cc.column_name) where c.table_name = UPPER(p_table) and c.constraint_type = ''P'' order by position) LOOP
l_sql := l_sql||CASE l_first_column WHEN FALSE THEN '','' END||pk.column_name||'' ''||get_data_type(pk.data_type,CASE WHEN pk.data_type = ''VARCHAR2'' THEN pk.data_length ELSE pk.data_precision END,pk.data_scale);
l_first_column := FALSE;
END LOOP;
END IF;
...
IF p_coldata(l_column).method IN (''C'',''T'') THEN
IF l_sql IS NULL THEN
l_sql := ''SELECT ''||case when p_parallel > 1 THEN ''/*+ parallel(t ''||p_parallel||'') */ '' end||''NULL FROM ''||get')||TO_CLOB('_temp_table_name(p_table,p_use_ctas)||'' t WHERE '';
END IF;
l_sql := l_sql||CASE l_first_column WHEN FALSE THEN '' OR '' END||get_temp_column_name(l_column,p_use_ctas)||'' IS NOT NULL'';
l_first_column := FALSE;
END IF;
...
l_sql := l_sql||'') SELECT ''||case when p_parallel > 1 THEN ''/*+ parallel(s ''||ceil(p_parallel/2)||'') */ '' end;
l_first_column := TRUE;
FOR pk IN (SELECT column_name FROM user_constraints c join user_cons_columns cc on (cc.constraint_n')||TO_CLOB('ame = c.constraint_name) where c.table_name = UPPER(p_table) and c.constraint_type = ''P'' order by position) LOOP
l_sql := l_sql||CASE l_first_column WHEN FALSE THEN '','' END||pk.column_name;
l_first_column := FALSE;
END LOOP;
l_column := p_coldata.FIRST;
...
debug_dtl_success;
EXCEPTION WHEN OTHERS THEN
debug_dtl_')||TO_CLOB('error(DBMS_UTILITY.FORMAT_ERROR_STACK||CHR(10)||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
RAISE DTL_EXCEPTION;
END;
END IF;
END populate_tmp;
...
l_column := p_coldata.NEXT(l_column);
END LOOP;
l_column := p_coldat')||TO_CLOB('a.FIRST;
l_first_col := TRUE;
WHILE l_column IS NOT NULL LOOP
IF p_coldata(l_column).data_type = ''VARCHAR2'' AND p_coldata(l_column).method = ''I'' AND p_coldata(l_column).data_precision_length < p_coldata(l_column).tmp_precision_length THEN
IF l_first_col THEN
...
debug_dtl_success;
EXCEPTION WHEN OTHERS THEN
debug_dtl_error(DBMS_UTILITY.FORMAT_ERROR_STACK||CHR(10)||DBMS_UTILITY.FORMAT_ERRO')||TO_CLOB('R_BACKTRACE);
RAISE DTL_EXCEPTION;
END;
END IF;
END modify_precisions;
...
l_first_column := FALSE;
END IF;
l_')||TO_CLOB('column := p_coldata.NEXT(l_column);
END LOOP;
l_sql := l_sql||'' FROM ''||p_table||'' o JOIN ''||get_temp_table_name(p_table,p_use_ctas)||'' n ON ('';
l_first_column := TRUE;
FOR pk IN (SELECT column_name FROM user_constraints c join user_cons_columns cc on (cc.constraint_name = c.constraint_name) where c.table_name = UPPER(p_table) and c.constraint_type = ''P'' order by position) LOOP
...
END LOOP;
BEGIN
debug_dtl(''Populate original columns from Out of place columns')||TO_CLOB(''',l_sql);
EXECUTE IMMEDIATE l_sql;
debug_dtl_success;
EXCEPTION WHEN OTHERS THEN
debug_dtl_error(DBMS_UTILITY.FORMAT_ERROR_STACK||CHR(10)||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
...
debug_dtl_success;
EXCEPTION WHEN OTHERS THEN
debug_dtl_error(DBMS_UTILITY.FORMAT_ERROR_STACK||CHR(10)||DBMS_UTILITY.FORMAT_ERROR_')||TO_CLOB('BACKTRACE);
RAISE DTL_EXCEPTION;
END;
END IF;
END;
...
IF p_coldata(l_column).method != ''N'' AND p_coldata(l_column).tmp_renamed != ''Y'' THEN
BEGIN
debug_dtl(''Renaming column ''||l_column||''->''||')||TO_CLOB('get_rename_column_name(l_column),''ALTER TABLE ''||p_table||'' RENAME COLUMN ''||l_column||'' TO ''||get_rename_column_name(l_column));
EXECUTE IMMEDIATE ''ALTER TABLE ''||p_table||'' RENAME COLUMN ''||l_column||'' TO ''||get_rename_column_name(l_column);
debug_dtl_success;
EXCEPTION WHEN OTHERS THEN
debug_dtl_error(DBMS_UTILITY.FORMAT_ERROR_STACK||CHR(10)||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
...
IF l_use_out_of_place AND (l_create_out_of_place OR l_populate_out_of_place OR l_null_orig) THEN
-- If we created or populated the Tmp Tbl/Col then clearly we need to null the original columns
-- But if Tmp was previously populated, and we haven''t yet modified the data type, then we will null the columns (same cost to null ')||TO_CLOB('as to query for values against all nulls)
set_orig_null(coldata, p_table);
ELSIF l_use_out_of_place THEN
-- defaults to TRUE.  The only time we may not need to populate is when out of place was already populated previously, i.e. we didn''t just set values to null
l_populate_orig := NOT is_orig_populated(p_coldata, p_table);
...
set_renamed(coldata, p_table_name);
set_method(coldata, p_table_name, p_increase_only, l_use_ctas, is_release);
validate_data(coldata, l')||TO_CLOB('_use_ctas);
modify_types(coldata, p_table_name, is_release AND p_set_unused);
debug_hdr_success;
EXCEPTION
WHEN DTL_EXCEPTION THEN
...
')),
	 ('sbmo.package.pkg_pdx_schema_updater_meta.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE "SBMO"."PKG_PDX_SCHEMA_UPDATER_META" AS
...
-- For example')||TO_CLOB(' ... Metadata stores TA12345 REQUIRES TA23456, TA23457, TA23458
...
AND file_hash = NVL(pkg_pdx_schema_up')||TO_CLOB('dater_meta.meta_hash(s.sql),0)
UNION
...
FROM (SELECT file_na')||TO_CLOB('me,
...
DELETE FROM pdx_schema_file_hash
WHERE file_name = (SELECT file_name ')||TO_CLOB('FROM pdx_schema_file_taskver WHERE task_version = p_task)
AND file_source = ''A'';
DELETE FROM pdx_schema_taskver_meta
...
END IF;
ELSE
FOR valrec IN (SELECT REGEXP_SUBSTR(l_data,''(^|,) *([^ ,]*) *'',1,level,''im'',2) val from dual connect by level <')||TO_CLOB('= l_valcount) LOOP
...
g_task_to_value(rec.TASK_VERSION)(''A'')(rec.META_TAG)(g_task_to_value(rec.TASK_VERSION)(''A'')(rec.META_TAG).LAST):=rec.TASK_')||TO_CLOB('VERSION_VALUE;
...
IF valrec.val IS NULL THEN
RAISE_APPLICATION_ERROR(-20011,l_meta||'' for ''||rec.task_version|')||TO_CLOB('|'' in schema_updater_sql is empty or invalid'');
ELSE
INSERT INTO PDX_SCHEMA_TASKVER_META ( TASK_VERSION, META_TAG, TASK_VERSION_VALUE, TASK_SOURCE )
...
g_value_to_task(rec.TASK_VERSION_VALUE)(''B'')(rec.META_TAG)(g_value_to_t')||TO_CLOB('ask(rec.TASK_VERSION_VALUE)(''B'')(rec.META_TAG).LAST):=rec.TASK_VERSION;
END LOOP;
END;
...
RETURN NULL;
END;
FUNCTION get_metadata ( p_file_or_task VARCHAR2, p_parameter VARC')||TO_CLOB('HAR2, p_source VARCHAR2 ) RETURN VARCHAR2 IS
l_task VARCHAR2(255);
l_cnt  NUMBER;
BEGIN
...
END PKG_PDX_SCHEMA_UPDATER_META;
')),
	 ('sbmo.package_body.pkg_pdx_schema_updater_meta.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SBMO"."PKG_PDX_SCHEMA_UPDATER_META" AS
g_captured BOOLEAN := FALSE;
--  TYPE value_tbl IS TABLE OF VARCHAR2(2000);
TYPE metatag_tbl IS TABLE OF results_tbl INDEX BY VARCHAR2(40);
...
UNION
SELECT file_name,
UPPE')||TO_CLOB('R(REGEXP_SUBSTR(file_name, lower(value) || ''_(.*?)_ddl.sql$'',1,1,''im'',1)),
NVL(UPPER(SUBSTR(CAST(REGEXP_SUBSTR(sql, ''^ *-- *(TASKFILE|RELEASEFILE|TASKREF|RELEASEREF) *:.*'',1,1,''im'',1) AS VARCHAR2(15)),1,1)),''O'')
FROM schema_updater_sql
LEFT OUTER JOIN pdx_schema_file_taskver USING (file_name)
JOIN pdx_schema_config ON (key = ''APPLICATIONPREFIX'')
...
l_value VARCHAR2(2000);
BEGIN
l_meta := g_task_to_value(p_task)')||TO_CLOB('(''A'').FIRST;
FOR ctr IN 1..g_task_to_value(p_task)(''A'').COUNT LOOP
l_idx1 := g_task_to_value(p_task)(''A'')(l_meta).FIRST;
FOR ctr1 IN 1..g_task_to_value(p_task)(''A'')(l_meta).COUNT LOOP
l_value := g_task_to_value(p_task)(''A'')(l_meta)(l_idx1);
...
END IF;
l_iter := l_iter + 1;
l_meta := SUBSTR(REGEXP_SUBSTR(rec.ddl,''^ *-- *(''||l_metatags||'') ')||TO_CLOB('*:.*$'',1,l_iter,''im'',1),1,30);
END LOOP;
IF rec.file_type = ''R'' THEN
l_iter := 1;
l_data := REGEXP_SUBSTR(rec.ddl,''^ *--\@(''||l_prefix||''_)?(.*?)(_ddl|_ddl\.sql)? *$'',1,l_iter,''im'',2);
...
IF rec.file_type = ''R'' THEN
l_iter := 1;
l_data := REGEXP_SUBSTR(re')||TO_CLOB('c.ddl,''^ *--\@(''||l_prefix||''_)?(.*?)(_ddl|_ddl\.sql)? *$'',1,l_iter,''im'',2);
WHILE l_data IS NOT NULL LOOP
IF get_taskname(l_data) IS NULL THEN
RAISE_APPLICATION_ERROR(-20011,''Unable to find taskname for ''||l_data||'' - is taskfile loaded?'');
ELSE
...
END PKG_PDX_SCHEMA_UPDATER_META;
')),
	 ('sbmo.package.pkg_pdx_schema_updater_rpt.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE "SBMO"."PKG_PDX_SCHEMA_UPDATER_RPT" AS
...
WHERE pdx_schema_upd_pkgcall_hist_id = p_call_id
AND pdx_schema_version_his')||TO_CLOB('tory_id = p_vers_id
ORDER BY id;
...
IF p_rec.error_message IS NOT NULL THEN
add(''          Message:  ''||S')||TO_CLOB('UBSTR(REGEXP_REPLACE(p_rec.error_message,''^'',''                    '',1,0,''m''),21));
END IF;
IF p_rec.sql_error_message IS NOT NULL THEN
add(''          SqlError: ''||SUBSTR(REGEXP_REPLACE(p_rec.sql_error_message,''^'',''                    '',1,0,''m''),21));
END IF;
...
END PKG_PDX_SCHEMA_UPDATER_RPT;
')),
	 ('sbmo.package_body.pkg_pdx_schema_updater_rpt.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SBMO"."PKG_PDX_SCHEMA_UPDATER_RPT" AS
FUNCTION schema_version RETURN VARCHAR2 IS BEGIN RETURN PKG_PDX_SCHEMA_UPDATER.schema_version; END;
FUNCTION updater_version RETURN VARCHAR2 IS BEGIN RETURN PKG_PDX_SCHEMA_UPDATER.updater_version; END;
FUNCTION report_version RETURN VARCHAR2 IS BEGIN RETURN ''1.00''; END;
...
PROCEDURE print_process ( p_rec process_cur%ROWTYPE ) IS
BEGIN
add(''Process:  ''||p_rec.process||'' Started [''||p_rec.start_date||''], Duration [')||TO_CLOB('''||(p_rec.duration)||'']'');
END;
BEGIN
OPEN call_cur;
FETCH call_cur INTO call_rec;
...
'));
INSERT INTO ALL_OBJECTS (FILE_NAME,DDL_CLOB) VALUES
	 ('sbmo.package.pkg_sbmo_purge.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE "SBMO"."PKG_SBMO_PURGE" 
AS
/*******************************************************************************
/*
/* NAME:		PKG_SBMO_PURGE
...
/******************************************************************************/
	...
	-- Values for scheduler job typ')||TO_CLOB('e
	...
	-----------------------------------------------------------------------------
	-- D')||TO_CLOB('efine global exception variables
	-----------------------------------------------------------------------------
	...
	p_Deli')||TO_CLOB('miter	IN		CHAR			DEFAULT '',''
	)
	RETURN tbl_Strings
	PIPELINED;
...
/******************************************************************************/
	...
	b_ParallelChunkCount		NUMB')||TO_CLOB('ER							:= NULL;
	...
	FUNCTION Fn_GetMonthsToKeep
	(
	p_Ta')||TO_CLOB('bleName	IN		VARCHAR2
	)
	RETURN NUMBER
	IS
		-- Processing Variables
	...
	/* Name					Date		Modifications
	/* -----------------	----------	-------------------------------------------
	/* Edward Stephenson	01/27/2019	C')||TO_CLOB('reated function
	/*
	/***************************************************************************/
	FUNCTION Fn_GetPurgeDate
	(
	...
	/*
	/* Description:	This function will retrieve the value of the configur')||TO_CLOB('ation
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, ')||TO_CLOB('exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Fn_GetConfig'', b_SqlCode, b_SqlErrm,
				''Other error'',
				''P_Name: '' || p_Name );
	...
	BEGIN
		...
		/********************************************************************/
		v_StartDate := to_date( to_char( v_RunTime, ''mm/dd/yyy')||TO_CLOB('y '' ) || v_StartHour, ''mm/dd/yyyy HH24mi'' );
	...
	EXCEPTION
		WHEN e_NotInWindow THEN
			...
			insert into purge_er')||TO_CLOB('ror_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
	...
	/***************************************************************************
	/*
')||TO_CLOB('	/* Name					Date		Modifications
	/* -----------------	----------	-------------------------------------------
	...
	BEGIN
		...
		/* If none found,')||TO_CLOB(' return the first one
		/********************************************************************/
		BEGIN
			select	table_name
			into	f_TableName
	...
	/***************************************************************************
	/*
	/* Name					Date		Modificati')||TO_CLOB('ons
	/* -----------------	----------	-------------------------------------------
	...
	/*
	/* Return:		VARCHAR2
	/*				++ Purge c')||TO_CLOB('olumn associated with the table passed in
	/*
	/***************************************************************************
	/*
	/* Name					Date		Modifications
	...
	/*				++ Table name used for the task name
	/*				p_PartName			IN		VARCHAR2
	/*				++ Partition name use')||TO_CLOB('d for task name
	/*
	...
	EXCEPTION
		WHEN e_BadJobType THEN
			...
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master')||TO_CLOB('_seq.nextval, systimestamp,
...
/***************************************************************************/
	...
	BEGIN
		...
		select	count(*)
		into	f_Count
		from	user_paral')||TO_CLOB('lel_execute_chunks
	...
	/*		')||TO_CLOB('		table into pieces for the parallel execution
	/*
	/* Notes:		None
	/*
	/* Return:		VARCHAR2
...
dbms_output.put_line(''DEBUG Fn_GetChunkSql : p_TableName - '' || p_TableName || '' , p_PartName - '' || p_PartName );
		...
		v_Sql := v_Sql || ''to_date( '''''' || to_char( p_PurgeDate, ''mm/dd/yyyy hh24:mi:ss'' ) || '''''', ''''mm/dd/yyyy hh24:mi:ss'''' ) '' ||
							...
							'') '' ||
					''group by rownum, id '' ||
					''ha')||TO_CLOB('ving mod(rownum, '' || b_ChunkSize || '' ) = 0 '' ||
					''union '' ||
...
dbms_output.put_line(''DEBUG | Fn_GetChunkSql - '' || v_Sql );
	...
	EXCEPTION
		WHEN OTHERS THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			DBMS_OUTPUT.PUT_LINE( ''Error in Fn_GetChunkSql for table '' || p_TableName || '' and partition '' || p_PartName || '' and column '' || p_ColumnName|| '' | p_PurgeDate: '' || to_char(p_PurgeDate,''mm/dd/yy')||TO_CLOB('yy hh24:mi:ss'') );
	...
	IS
		...
		v_Sql				VARCHAR2(4000)		:= NULL;
		v')||TO_CLOB('_part_clause   	VARCHAR2(4000)		:= NULL;
	...
	BEGIN
		...
		if(	p_Tenant is NOT NULL ) then
			...
			v_FkSq')||TO_CLOB('l := v_FkSql || ''and '' || p_ColumnName || '' < '' ||
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE( ''Sql thu')||TO_CLOB('s far: '' || v_Sql );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
	...
	EXCEPTION
		...
		WHEN e_InvalidJobClass THEN
			...
			values( master_seq.nextval, systimestamp,
				...
				''None'' );
			commit;
			raise_application')||TO_CLOB('_error(-20011, ''Job class does not exist or user has no permissions on it: '' || b_JobClass );
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			rollback;
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_me')||TO_CLOB('ssage, parameter_values)
			values( master_seq.nextval, systimestamp,
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert')||TO_CLOB(' into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
	...
	EXCEPTION
		WHEN e_StopRunning THEN
			...
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error')||TO_CLOB('_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
	...
	/*				++ purge_history_partitions i')||TO_CLOB('d
	/*				p_ExecJob		IN		CHAR
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id')||TO_CLOB(', error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
	...
	EXCEPTION
		...
		WHEN e_StopRunning THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			i')||TO_CLOB('nsert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Sp_CheckWindowEndTime'',  b_SqlCode, b_SqlErrm,
	...
	/*
	/***************************************************************************/
	PROCEDURE Sp_C')||TO_CLOB('reateMonitorJob
	(
	...
	EXCEPTION
		...
		WHEN e_BadJobType THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			insert into purge_error_log(id, error_time, code_block, sql_code, ')||TO_CLOB('sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
	...
	PROCEDURE Sp_DeleteFKs
	(
	p_')||TO_CLOB('Hist			IN OUT	purge_history_partitions%ROWTYPE,
	p_PurgeDate		IN		DATE,
	p_Tenant		IN		NUMBER
	)
	IS
...
and c.table_name = p.table_name;
	...
	BEGIN
		...
		/********************************************************************/
		p_Hist.fk_sql := null;
		p_Hist.child_fk_sql')||TO_CLOB(' := null;
		...
		for lr in c_Fks( p_Hist.table_name ) loop
		   ...
		   if ( p_Hist.partition_name = ''NON_PARTITIONED'' ) then
			...
			v_DelSql := ''delete /*+ parallel (d,'' || b_ParallelQueryCount || '') */ '' ||
				')||TO_CLOB('	''from '' || p_Hist.table_name ||  v_part_clause || '' d '' ||
...
dbms_output.put_line (''Inside spdelete - v_DelSql : '' || v_DelSql);
			...
			exception
				...
				dbms_output.put_line (''Debug : Inside e_ChildRecFound exception '');
						...
						dbms_')||TO_CLOB('output.put_line (''Debug : Inside c_ChildFks loppop '');
...
dbms_output.put_line (''Inside spdelete 2 - v_ChildSql : '' || v_ChildSql);
							...
							exception						
								...
								/********************************************/
								when e_ChildR')||TO_CLOB('ecFound then
									for lr2 in c_GrandChildfks( lr1.table_name ) loop	
									  if ( lr2.partitioned = ''YES'') then
										...
										v_GrandChildSql := v_GrandChildSql ||	''to_date('''''' || to_char( p_PurgeDate, ''mm/dd/yyyy hh24:mi:ss'') || ''')||TO_CLOB(''''', ''''mm/dd/yyyy hh24:mi:ss'''') '' ||
															'') '' ||
														'') '' ||
													'') '';
			...
			if( v_TestDate > p_PurgeDate ) then
				...
				exception
					when no_data_found then
						...
						execute immediate ''alter session set')||TO_CLOB(' optimizer_mode = '' || b_SubpartOptMode || '' '';
						v_StartDate := sysdate;
						execute immediate v_Sql;
...
--Medha : This is needed only incase table is partitioned
		...
		Sp_CreateTask( v_TaskNa')||TO_CLOB('me );
		/********************************************************************/
...
dbms_output.put_line( ''EHS: Calling Sp_DeleteFKs - v_HistoryRec.id- ''  || v_HistoryRec.id || ''p_TableName'' || p_TableName );
		...
		update purge_history_partitions')||TO_CLOB(' set row = v_HistoryRec where id = v_HistoryRec.ID;
		commit;
	EXCEPTION
		...
		WHEN e_FinishedErrors THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm ')||TO_CLOB(':= substr(SQLERRM,1,250);
			v_HistoryRec.STATUS := Fn_StatusNbrToChar( v_Status );
			Sp_SumChunksToParts( v_HistoryRec );
		...
		WHEN e_FailedTask THEN
			...
			values( master_seq.nextval, systimestamp,
				''Sp_PurgePartition'',  b_SqlCode, b_SqlErrm,
				''Task failed with status '' || v_')||TO_CLOB('HistoryRec.STATUS,
				''p_TableName: '' || p_TableName || '' | p_PartName: '' || p_PartName || '' | p_PurgeColumn: '' || p_PurgeColumn || '' | p_HistId: '' || p_HistId || '' | p_PurgeDate: '' || to_char( p_PurgeDate, ''mm/dd/yyyy hh24:mi:ss'' ) || '' and tenant id '' || nvl(v_Tenant, -1) );
			commit;
			RAISE e_FailedTask;
		WHEN OTHERS THEN
	...
	BEGIN
		...
		if( v_HistoryRec.Status = c_FkDelete ) THEN
			...
			Sp_DeleteFKs( v_HistoryRec, p_')||TO_CLOB('PurgeDate, v_Tenant );
	...
	EXCEPTION
		...
		WHEN e_SubPartDrop THEN
			...
			v_HistoryRec.STATUS := c_SubPartDrop;
			Sp_SumChunksToParts( v_HistoryRec );
			update purge_history_partitions set row = v_HistoryRec where id = v_History')||TO_CLOB('Rec.ID;
			commit;
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
		...
		WHEN e_StopRunning THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			DBMS_OUTPUT.PUT_LINE( ''Sp_RestartPurgePartition: Stop run requested '' || v_HistoryRec.table')||TO_CLOB('_name || '' and partition '' || v_HistoryRec.partition_name );
			v_HistoryRec.STATUS := c_Stopped;
			Sp_SumChunksToParts( v_HistoryRec );
			update purge_history_partitions set row = v_HistoryRec where id = v_HistoryRec.ID;
			commit;
		...
		WHEN e_FailedTask THEN
			...
			DBMS_OUTPUT.PUT_LINE( ''   select * from table(pkg_sbmo_purge.fn_showe')||TO_CLOB('rrors('' || b_ErrDate || '')); '' );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			update purge_history_partitions set row = v_HistoryRec where id = v_HistoryRec.ID;
			commit;
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
	...
	/*
	/* Parameters:	p_PurgeDate		DATE
	/')||TO_CLOB('*				++ Date used as the purge baseline
	/*
...
')||TO_CLOB('			rollback;
	...
	BEGIN
		...
		/********************************************************************/
		CASE	p_Status
				WHEN dbms_parallel_execute.create')||TO_CLOB('d				
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Sp_StopRunningPurge'',  b_SqlCode, b_Sq')||TO_CLOB('lErrm,
				''Other error'',
				''None'' );
			commit;
			RAISE;
	...
	BEGIN
		...
		WHILE ( v_Retries < b_RetryCount and v_Status in ( dbms_parallel_execute.processing, dbms_parallel_execute.processed ) ) LOOP
			if( v_Status = dbms_parallel_execute.processing ) then
				dbms_lock.s')||TO_CLOB('leep( b_SleepTime );
			else
	...
	EXCEPTION
		...
		WHEN e_FailedTask THEN
			...
			insert into purge_error_log(id, error_time, code')||TO_CLOB('_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Sp_MonitorTask'',  b_SqlCode, b_SqlErrm,
	...
	IS
		...
		v_PurgeColumn		VARCHAR2(30)					:= NUL')||TO_CLOB('L;
...
order by partition_name;
	...
	BEGIN
		...
		/* again p')||TO_CLOB('ick up where we left off
		/********************************************************************/
...
dbms_output.put_line(''DEBUG | Restart Stopped task from previous run, task status was -'' || v_HistoryRec.STATUS );
			...
			for lr in c_CatchUpParts( v_HistoryRec.ID, v_HistoryRec.TABLE_NAME ) loop
				...
				dbms_output.put_line(''DEBUG | v_HistoryRec.table_name-'' || v_HistoryRec.table_name || ''lr.partition_name - '' || lr.partition')||TO_CLOB('_name || ''v_PurgeColumn -'' || v_PurgeColumn || ''v_HistoryRec.ID - '' || v_HistoryRec.ID || ''v_HistoryRec.PURGE_DAT-'' || v_HistoryRec.PURGE_DATE );
		...
		while v_TableCount < v_EligibleCount loop
		   ...
		   dbms_output.put_line (''DEBUG| Before call First table to purge  TABLE_NAME - '' || v_HistoryRec.TABLE_NAME  );
			...
			/****************************************************************/
			/* Set the initi')||TO_CLOB('al history record values
			/****************************************************************/
	...
	EXCEPTION
		...
		WHEN e_FkDelete THEN
			...
			Sp_SetRunFlag( c_No );
			v_HistoryRec.Status := c_FkDelete;
			v_HistoryRec.TOT_ELAPSED_SECS := Fn_')||TO_CLOB('GetTaskElapsedSecs( v_HistoryRec.ID );
			update purge_history set row = v_HistoryRec where id = v_HistoryRec.ID;
			commit;
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
		...
		WHEN e_StopRunning TH')||TO_CLOB('EN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			Sp_SetRunFlag( c_No );
			v_HistoryRec.Status := c_Stopped;
		...
		WHEN ')||TO_CLOB('e_FailedTask THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
...
')),
	 ('sbmo.package_body.pkg_sbmo_purge.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SBMO"."PKG_SBMO_PURGE" 
AS
/*******************************************************************************
/*
...
/******************************************************************************/
	...
	/*				++ Delimiter to split the string on
	/*
	/* De')||TO_CLOB('scription:	This function will split the string and return the values
	/*				as a row of data
	/*
	/* Notes:		None
	/*
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( mas')||TO_CLOB('ter_seq.nextval, systimestamp,
				''Fn_SplitString'', b_SqlCode, b_SqlErrm,
				''Other error'',
				''P_String: '' || p_String || '' | p_Delimiter: '' || p_Delimiter );
			commit;
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE( ''Error in Fn_KeepRunning'' );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exce')||TO_CLOB('ption_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Fn_KeepRunning'', b_SqlCode, b_SqlErrm,
				''Other error'',
				''None'' );
	...
	EXCEPTION
		WHEN OTHERS THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			DBMS_OUTPUT.PUT_LINE( ''Erro')||TO_CLOB('r in Fn_GetMonthsToKeep for '' || p_TableName );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Fn_GetMonthsToKeep'', b_SqlCode, b_SqlErrm,
	...
	EXCEPTION
		WHEN OTHERS THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			DBMS_OUT')||TO_CLOB('PUT.PUT_LINE( ''Error in Fn_GetPurgeDate for p_TableName: '' || p_TableName );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Fn_GetPurgeDate'', b_SqlCode, b_SqlErrm,
	...
	EXCEPTION
		WHEN NO_DATA_FOUND THEN
			...
			b_SqlErrm := substr(SQLERRM,1,250);
			DBMS_OUTPUT.PUT_LINE( p_Name || '' not found in the v$parameter table '' );
')||TO_CLOB('			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Fn_GetDbParameter'', b_SqlCode, b_SqlErrm,
				''No data found in the v$parameter table for the name passed in'',
				''P_Name: '' || p_Name );
	...
	BEGIN
		...
		/* greater than the end date increment the end date to tomorrow
		/********************************************************************/
		if( v_RunTime < v')||TO_CLOB('_StartDate ) then
			if( v_RunTime > v_StartDate-1 and v_RunTime < v_EndDate-1 ) then
				v_StartDate := v_StartDate - 1;
				v_EndDate := v_EndDate - 1;
			else
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE( ''Error in Fn_GetEndWindow using '' || v_WindowTimes );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sq')||TO_CLOB('l_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Fn_GetEndWindow'', b_SqlCode, b_SqlErrm,
				''Other error'',
				''None'' );
	...
	/* Notes:		None
	/*
	/* Return:		PURGE_HISTORY_PARTITIO')||TO_CLOB('NS%ROWTYPE
	/*				++ The latest history row in purge_history
	/*
	/***************************************************************************
	/*
	...
	/***************************************************************************/
	/*
	/* Function:')||TO_CLOB('	Fn_GetNextPurgeTable
	/*
	/* Parameters:	p_TableName      IN    VARCHAR2
	/*				++ Last purge table
	/*
	...
	EXCEPTION
		WHEN OTHERS THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			DBMS_OUT')||TO_CLOB('PUT.PUT_LINE( ''Error in Fn_GetNextPurgeTable for '' || p_TableName );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Fn_GetNextPurgeTable'', b_SqlCode, b_SqlErrm,
	...
	EXCEPTION
		WHEN OTHERS THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(')||TO_CLOB('SQLERRM,1,250);
			DBMS_OUTPUT.PUT_LINE( ''Error in Fn_GetPurgePosition for '' || p_TableName );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
	...
	BEGIN
		...
		/********************************************************************/
		/* Return the value t')||TO_CLOB('o the calling procedure
		/********************************************************************/
		if( f_Parallel <= b_ParallelChunkCount ) then
			RETURN f_Parallel;
		else
	...
	BEGIN
		...
		into	f_Column
		from	purge_tables
		where	tab')||TO_CLOB('le_name = p_TableName;
	...
	BEGIN
		...
		/* Generate the task name
		/********************************************************************/
		v_Name := c_TaskNamePrefix || ''_PP'' || fn_GetPurgePosition( p_Table')||TO_CLOB('Name );
		v_Name := v_Name || ''_'' || p_PartName;
		/********************************************************************/
		/* Return the value to the calling procedure
		/********************************************************************/
...
')||TO_CLOB('/***************************************************************************/
	...
	/***************************************************************************/
	/*
	/* Function:	Fn_Get')||TO_CLOB('ConsColumns
	/*
	/* Parameters:	p_TableName			IN		VARCHAR2
	/*				++ Table associated with the constraint
	/*				p_ConsName			IN		VARCHAR2
...
dbms_output.put_line(''DEBUG Fn_GetChunkSql : p_TableName - '' || p_TableName || '' , p_PartName - '' || p_PartName );
		...
		-- Do not include tenant_id, testing determined it is faster for the chunking
		-- sql to just do the subpartition pruning and then full subpartition scan
		v_Sql := v_Sql || '' where '' || p_Co')||TO_CLOB('lumnName || '' < '';
		v_Sql := v_Sql || ''to_date( '''''' || to_char( p_PurgeDate, ''mm/dd/yyyy hh24:mi:ss'' ) || '''''', ''''mm/dd/yyyy hh24:mi:ss'''' ) '' ||
					''order by 1 '' ||
				''), '' ||
				''chunks '' ||
...
dbms_output.put_line(''DEBUG | Fn_GetChunkSql - '' || v_Sql );
	...
	/*				++ Table to chunk sql for
	/*				p_PartName			IN		VARCHAR2
	/*		')||TO_CLOB('		++ Partition to chunk sql for
	/*				p_ColumnName		IN		VARCHAR2
	/*				++ Date column used to purge by
	/*				p_PurgeDate			IN		DATE
	/*				++ Date to purge below
	...
	BEGIN
		...
		v_part_clause := null;
		  ...
		  v_part_clause := '' partition ')||TO_CLOB('('' || p_PartName || '') '' ;
		end if;
		...
		v_Sql := v_Sql || ''to_date( '''''' || to_char( p_PurgeDate, ''mm/dd/yyyy hh24:mi:ss'' ) || '''''', ''''mm/dd/yyyy hh2')||TO_CLOB('4:mi:ss'''' ) '' ||
						''and id between :start_id and :end_id ; '' ||
						'' '' ||
						''v_count := sql%rowcount; '' ||
						'' '' ||
	...
	/*
	/* Description:	This function will get the tenant_id associated with the
	/*				t')||TO_CLOB('able and partition unless the partition is other then it
	/*				will return null
	/*
	/* Notes:		None
	/*
	...
	EXCEPTION
		WHEN NO_DATA_FOUND THEN
			...
			DBMS_OUTPUT.PUT_LINE( ''No data found in Fn_GetTenantId for table '' || p_TableName || '' and partition '' || p_PartName );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_cod')||TO_CLOB('e, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Fn_GetTenantId'',  b_SqlCode, b_SqlErrm,
				''No Data Found'',
				''p_TableName: '' || p_TableName || '' | p_PartName: '' || p_PartName );
	...
	BEGIN
		...
		b_DefPartMins := to_number( Fn_GetConfig( c_DefPartMins ) );
		b_ChunkSize	:= to_number( Fn_GetConfig( c_ChunkSize ) );
		b_ParallelChunkCount	')||TO_CLOB(':= to_number( Fn_GetConfig( c_ParallelChunkCount ) );
		b_ParallelQueryCount	:= to_number( Fn_GetConfig( c_ParallelQueryCount ) );
		b_RetryCount := to_number( Fn_GetConfig( c_RetryCount ) );
		b_SleepTime := to_number( Fn_GetConfig( c_SleepTime ) );
		b_MultiBlock := to_number( Fn_GetConfig( c_MultiBlock ) );
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			...
			b_SqlErrm := substr(SQLERRM,1,250);
			DBMS_OUTPUT.PUT_LINE( ''Error in Sp_SetConfig'' );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );')||TO_CLOB('
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Sp_SetConfig'',  b_SqlCode, b_SqlErrm,
				''Other error'',
	...
	BEGIN
		...
		exception
			when others then
				v_Sql := ''Create or replace directory dba_log_dir as ''''/oracl')||TO_CLOB('e_scripts/log/'' || sys_context(''USERENV'',''INSTANCE_NAME'') || '''''' '';
				execute immediate v_Sql;
				v_File := utl_file.fopen( ''DBA_LOG_DIR'', v_FileName, ''w'', 32760 );
		end;
		/********************************************************************/
	...
	BEGIN
		...
		if( v_Count > 0 ) then
			...
			UTL_SMTP.WRITE_DATA( v_MailConn, ''To: '' || b_MailTo || UTL_TCP.crlf );
			UTL_SMTP.WRITE_DATA( v_MailConn, ''From: '' || b_MailFrom || UTL_TCP.crlf );
			UTL_SMTP.WRITE_DATA( v_MailConn, ''Subject: '' || v_Subj || U')||TO_CLOB('TL_TCP.crlf );
			UTL_SMTP.WRITE_DATA( v_MailConn, ''Reply-To: '' || b_MailFrom || UTL_TCP.crlf || UTL_TCP.crlf );
			UTL_SMTP.WRITE_DATA( v_MailConn, p_Msg );
			UTL_SMTP.CLOSE_DATA( v_MailConn );
			UTL_SMTP.QUIT( v_MailConn );
	...
	BEGIN
		...
		/* Send the report
		/********************************************************************/
		Sp')||TO_CLOB('_SendEmail( ''Daily SBMO Purge Report - '' || to_char(sysdate, ''dd Mon yyyy''), v_Report );
	...
	/*
	/***************************************************************************/
	PROCEDURE Sp_CheckMonitorReturn')||TO_CLOB('
	IS
		-- Processing Variables
		-- Fetch Variables
		f_Running			purge_run_check.purge_running%TYPE	:= null;
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			...
			b_SqlErrm := substr(SQLERRM,1,250);
			DBMS_OUTPUT.PUT_LINE( ''Error in Sp_CheckMonitorReturn'' );
			DBMS_OUTPUT.PU')||TO_CLOB('T_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Sp_CheckMonitorReturn'',  b_SqlCode, b_SqlErrm,
				''Other error'',
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE( ''Error in Sp_SumChunksToParts for History ID '' || p_HistRec.ID || '' and table '' || p_HistRec.table_name || '' and partition '' || p_HistRec.partition_name );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, ex')||TO_CLOB('ception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Sp_SumChunksToParts'', b_SqlCode, b_SqlErrm,
				''Other error'',
				''ID '' || p_HistRec.ID || '' and table '' || p_HistRec.table_name || '' and partition '' || p_HistRec.partition_name );
	...
	IS
		-- Processing Variables
		')||TO_CLOB('v_Sql				VARCHAR2(4000)		:= NULL;
		-- Fetch Variables
		-- Exception Variables
		-- Cursors
		cursor	c_bad_subparts
	...
	BEGIN
		...
		if(( p_PartName is null ) or (p_PartName = ''NON_PARTITIONED'' )) then
			for lr i')||TO_CLOB('n c_bad_ind( p_TableName ) loop
				v_Sql := v_Sql || ''execute immediate ''''alter index '' || lr.index_name ||
					'' rebuild '' ||
					'' online parallel '' || b_ParallelQueryCount || '' ''''; '';
			end loop;
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Sp_RebuildIndexes'',  b_SqlCode, b_SqlErrm')||TO_CLOB(',
				''Other error'',
				''p_TableName: '' || p_TableName || '' | p_PartName '' || p_PartName || '' | p_HistId: '' || p_HistId || '' | p_HistPartId: '' || p_HistPartId || '' | p_Sql: '' || p_Sql );
			commit;
			RAISE;
	...
	/*
	/* Notes:		If there is no data in the history tables to determine the
	/*				interval a default valu')||TO_CLOB('e from config settings will be used
	/*				This value is set at package instantiation
	/*
	/***************************************************************************
	/*
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			...
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, ')||TO_CLOB('systimestamp,
				''Sp_CheckWindowEndTime'',  b_SqlCode, b_SqlErrm,
				''Other error'',
				''p_TableName: '' || p_TableName || '' | p_PartName: '' || p_PartName );
			commit;
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			...
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
	')||TO_CLOB('			''Sp_CreateTask'',  b_SqlCode, b_SqlErrm,
				''Other error'',
				''p_TaskName: '' || p_TaskName );
			commit;
			RAISE;
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			values( master_seq.nextval, systimestamp,
				''Sp_ExecuteTask'',  b_SqlCode, b_SqlErrm,
				''Other error'',
				''p_TaskName: '' || p_TaskName || '' | p_Sql: '' || p_Sql || '' | ')||TO_CLOB('p_TableName: '' || p_TableName || '' | v_ParallelCount: '' || v_ParallelCount );
			commit;
			RAISE;
	END Sp_ExecuteTask;
	/***************************************************************************/
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			...
			values( master_seq.nextval, systimestamp,
				''Sp_CreateMonitorJob'',  b_SqlCode, b_SqlErrm,
				''Other error'',
			')||TO_CLOB('	''p_TaskName: '' || p_TaskName || '' | p_TableName: '' || p_TableName || '' | p_PartName: '' || p_PartName || '' v_Sql: '' || v_Sql );
			commit;
			RAISE;
	END Sp_CreateMonitorJob;
	/***************************************************************************/
	...
	IS
		...
		is
				...
				where   c.table_name = p_Table
						and c.constraint_type = ''R''
						and c.r_constraint_name = f.constraint_name
						and f.tabl')||TO_CLOB('e_name = p.table_name;
		cursor	c_Childfks
				(
				p_Table		VARCHAR2
				)
...
and c.table_name = p.table_name;
	...
	BEGIN
		...
		for lr in c_Fks( p_Hist.table_name ) loop
			...
			v_PurgeColumn := Fn_GetPurgeColumn( lr.table_name );
			/****************************************************************/
			/* Check if the partition exists in the')||TO_CLOB(' parent table, if not use
			/* the other partition
			/****************************************************************/
	       -- Medha : Check if table is partitioned or not, if not , then set partition_name to ''NON_PARTITIONED''
		   if ( p_Hist.partition_name = ''NON_PARTITIONED'' ) then
			...
			if( p_Tenant is NULL ) then
				...
				v_DelSql := v_DelSql || '' where tenant_id = '' || p_Tenant || '' and '' || v_Purg')||TO_CLOB('eColumn || '' < '';
			end if;
			v_DelSql := v_DelSql ||	''to_date('''''' || to_char( p_PurgeDate, ''mm/dd/yyyy hh24:mi:ss'') || '''''', ''''mm/dd/yyyy hh24:mi:ss'''') '' ||
								'') '' ||
							'') '';
						...
						if (lr1.PARTITIONED = ''YES'') then
							...
							v_ChildSql := ''delete /*+ parallel (d,'' || b_ParallelQueryCount || '') */ '' ||
											...
											'' where ('' || Fn_GetConsColumns( lr1.table_name, lr1.constraint_name ) || '') in '' ||
													...
													'' where ('' || Fn_GetConsColumns( p_Hist.table_name, lr.constraint_name ) || '') in '' ||
															''( '' ||
															''select	/*+ ')||TO_CLOB('parallel(r,'' || b_ParallelQueryCount || '') */ '' ||
																Fn_GetConsColumns( lr.table_name, lr.r_constraint_name ) || '' '' ||
															''from '' || lr.table_name || v_part_clause || '' r '';
dbms_output.put_line (''Inside spdelete 2 - v_ChildSql : '' || v_ChildSql);
							if( p_Tenant is NULL ) then
										...
										v_GrandChildSql := ''delete /*+ parallel (d,'' || b_ParallelQueryCount || '') */ '' ||
												...
												'' where ('' || Fn_GetConsColumns( lr2.table_name, lr2.constraint_name ) || '') in '' ||
													...
													''select /*+ parallel(c,'' || b_ParallelQueryCount || '') */ '' ||
														Fn_GetConsColumns( lr1.table_name, lr2.r_constraint_name ) || '' '' ||
													''from '' || lr1.table_name || v_part_clause || '' c '' ||
													'' where ('' || Fn_GetConsColumns( lr1.table_name, lr1.constraint_nam')||TO_CLOB('e ) || '') in '' ||
														''( '' ||
														''select /*+ parallel(t,'' || b_ParallelQueryCount || '') */ '' ||
															Fn_GetConsColumns( p_Hist.table_name, lr1.r_constraint_name ) || '' '' ||
														''from '' || p_Hist.table_name || v_part_clause || '' t '' ||
			...
			p_Hist.tot_elapsed_fk_secs := nvl(p_Hist.tot_elapsed_fk_secs,0) + Fn_GetElapsedSecs( v_StartDate, SYSDATE );
			commit;
	')||TO_CLOB('	end loop; -- for lr in c_Fks
	EXCEPTION
		WHEN e_EndWindow THEN
			raise e_FkDelete;
		WHEN e_StopRunning THEN
	...
	BEGIN
		...
		for lr in c_Subs( p_HistRec.table_name, p_HistRec.partition_name ) loop
			...
			/****************************************************************/
			if( mod( c_Subs%ROWCOUNT, 2 ) = 0 ) then
				Sp_CheckWindowEndTime( p_HistRec.tabl')||TO_CLOB('e_name, p_HistRec.partition_name );
			end if;
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			DBMS_OUTPUT.PUT_LINE( ''Error in Sp_DropSubpartitions for Table: '' || p_HistRec.table_name || '' Partition: '' || p_HistRec.partition_name ')||TO_CLOB(');
			DBMS_OUTPUT.PUT_LINE( ''v_Sql: '' || v_Sql );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
	...
	BEGIN
		/********************************************************************/
		/* Make sure we have time to start purgin')||TO_CLOB('g the partition
		/********************************************************************/
		dbms_output.put_line ( ''DEBUG - Sp_PurgePartition - Begins '' );
...
--Medha : This is needed only incase table is partitioned
		...
		/* Set the multiblock parameter to help speed up the I/O
		/********************************************************************/
		v_HistoryRec.chunk_sql := Fn_GetChunkSql( p_TableName, p_PartName, p_PurgeColumn, p')||TO_CLOB('_PurgeDate, v_Tenant );
		execute immediate ''alter session set db_file_multiblock_read_count='' || b_MultiBlock;
		Sp_ChunkTask( v_TaskName, v_HistoryRec.chunk_sql );
		execute immediate ''alter session set db_file_multiblock_read_count='' || b_OrigMultiBlock;
		v_Status := dbms_parallel_execute.task_status( v_TaskName );
		...
		if( v_Status = dbms_parallel_execute.no_chunks ) then
			...
			if( v_Status = dbms_parallel_execute.crashed ) then
				...
				raise e_Finish')||TO_CLOB('edErrors;
			elsif( v_Status <> dbms_parallel_execute.finished ) then
				raise e_FailedTask;
			end if;
...
dbms_output.put_line( ''EHS: Calling Sp_DeleteFKs - v_HistoryRec.id- ''  || v_HistoryRec.id || ''p_TableName'' || p_TableName );
	...
	EXCEPTION
		WHEN e_SubPartDrop THEN
			...
			values( master_seq.nextval, systimestamp,
				''Sp_PurgePartition'',  b_SqlCode, b_SqlErrm,
				''End of window while deleting Subpartitions'',
				''p_TableName: '' || p_TableName || '' | p_PartName: '' || p_PartName || '' | p_PurgeColumn: '' || p_PurgeColumn || '' | ')||TO_CLOB('p_HistId: '' || p_HistId || '' | p_PurgeDate: '' || to_char( p_PurgeDate, ''mm/dd/yyyy hh24:mi:ss'' ) || '' and tenant id '' || nvl(v_Tenant, -1) );
			commit;
			RAISE e_SubPartDrop;
		WHEN e_FkDelete THEN
			b_SqlCode := SQLCODE;
		...
		WHEN e_EndWindow THEN
			...
			values( master_seq.nextval, systimestamp,
				''Sp_PurgePartition'',  b_SqlCode, b_SqlErrm,
				''End of window'',
				''p_TableName: '' || p_TableName || '' | p_PartName: '' || p_PartName || '' | p_PurgeColumn: '' || p_PurgeColumn || '' | p_HistId: '' || p_HistId || '' | p_PurgeDate: '' || to_char( p_PurgeDate, ''mm/dd/yy')||TO_CLOB('yy hh24:mi:ss'' ) || '' and tenant id '' || nvl(v_Tenant, -1) );
			commit;
			RAISE e_EndWindow;
		WHEN e_StopRunning THEN
			b_SqlCode := SQLCODE;
		...
		WHEN e_FinishedErrors THEN
			...
			values( master_seq.nextval, systimestamp,
				''Sp_PurgePartition'',  b_SqlCode, b_SqlErrm,
				''Task finished with errors'',
				''p_TableName: '' || p_TableName || '' | p_PartName: '' || p')||TO_CLOB('_PartName || '' | p_PurgeColumn: '' || p_PurgeColumn || '' | p_HistId: '' || p_HistId || '' | p_PurgeDate: '' || to_char( p_PurgeDate, ''mm/dd/yyyy hh24:mi:ss'' ) || '' and tenant id '' || nvl(v_Tenant, -1) );
			commit;
			RAISE e_FinishedErrors;
		WHEN e_FailedTask THEN
			b_SqlCode := SQLCODE;
		...
		WHEN OTHERS THEN
			...
			commit;
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
		')||TO_CLOB('	values( master_seq.nextval, systimestamp,
				''Sp_PurgePartition'',  b_SqlCode, b_SqlErrm,
				''Other error'',
				''p_TableName: '' || p_TableName || '' | p_PartName: '' || p_PartName || '' | p_PurgeColumn: '' || p_PurgeColumn || '' | p_HistId: '' || p_HistId || '' | p_PurgeDate: '' || to_char( p_PurgeDate, ''mm/dd/yyyy hh24:mi:ss'' ) || '' and tenant id '' || nvl(v_Tenant, -1) );
			commit;
	...
	BEGIN
		...
		/********************************************************************/
		begin
			v_HistoryRec := Fn_GetLatestHistoryPartitions( p_HistI')||TO_CLOB('d );
		exception
			when no_data_found then
				raise e_NoPartitionToRestart;
		end;
		...
		if( v_HistoryRec.Status = c_FkDelete ) THEN
			...
			/* Then error so we don''t attempt to continue this task
			/****************************************************************/
			Sp_PurgePartition( v_HistoryRec.table_name, v_HistoryRec.partition_name, v_PurgeColumn, p_HistId')||TO_CLOB(', p_PurgeDate );
			raise e_NoPartitionToRestart;
		elsif( v_HistoryRec.STATUS in ( c_Stopped, c_Chunked, c_Crashed, c_Begin ) ) then
			/****************************************************************/
			/* Set the number of chunks and ensure we have time to start the
		...
		/********************************************************************/
		/* Complete th')||TO_CLOB('e history record and insert it
		/********************************************************************/
		Sp_RebuildIndexes( v_HistoryRec.table_name, v_HistoryRec.partition_name, v_HistoryRec.index_sql, v_HistoryRec.id_purge_history, v_HistoryRec.id );
		Sp_DropSubpartitions( v_HistoryRec, p_PurgeDate );
		v_HistoryRec.STATUS := Fn_StatusNbrToChar( v_Status );
	...
	EXCEPTION
		...
		WHEN e_FkDelete THEN
			...
			update purge_history_partitions set row = v_HistoryRec where id = v_HistoryRec.ID;
			commit;
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_va')||TO_CLOB('lues)
			values( master_seq.nextval, systimestamp,
				''Sp_RestartPurgePartition'',  b_SqlCode, b_SqlErrm,
				''End of window while deleting FKs'',
				''p_HistId: '' || p_HistId );
		...
		WHEN e_FinishedErrors THEN
			...
			v_HistoryRec.STATUS := Fn_StatusNbrToChar( v_Status );
			Sp_SumChunksToParts( v_HistoryRec );
			DBMS_OUTPUT.PUT_LINE( ''Error in Sp_RestartPurgePartition task finished with errors '' || v_HistoryRec.table_name || '' and partition '' || v_History')||TO_CLOB('Rec.partition_name );
			DBMS_OUTPUT.PUT_LINE( ''   select * from table(pkg_sbmo_purge.fn_showerrors('' || b_ErrDate || '')); '' );
			DBMS_OUTPUT.PUT_LINE( SQLERRM );
			update purge_history_partitions set row = v_HistoryRec where id = v_HistoryRec.ID;
			commit;
		...
		WHEN e_InvalidRestartStatus THEN
			...
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
			')||TO_CLOB('	''Sp_RestartPurgePartition'',  b_SqlCode, b_SqlErrm,
				''Restart has invalid status '' || v_HistoryRec.STATUS,
				''p_HistId: '' || p_HistId );
			commit;
			Raise e_InvalidRestartStatus;
	...
	/* Name					Date		Modifications
	/* -----------------	----------	-------------------------------------------
	/* Edward Stephenson	01/25/2019	Creat')||TO_CLOB('ed function
	/*
	/***************************************************************************/
	FUNCTION Fn_DailyRpt
	(
	...
	BEGIN
		...
		v_StartDate := to_date(to_char( sysdate, ''mm/dd/yyyy '' ) || v_StartHour || ''01'', ''mm/dd/yyyy hh24miss'');
		if( v_StartDate < sysdate ) then
			v_StartDate := v_St')||TO_CLOB('artDate + 1;
		end if;
	...
	EXCEPTION
		...
		WHEN e_EndWindow THEN
			...
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, para')||TO_CLOB('meter_values)
			values( master_seq.nextval, systimestamp,
				''Sp_MonitorTask'',  b_SqlCode, b_SqlErrm,
				''Task stopped due to end of window condition '',
				''p_TaskName: '' || p_TaskName || '' | p_TableName: '' || p_TableName || '' | p_PartName: '' || p_PartName );
		...
		WHEN OTHERS THEN
			...
			values( master_seq.nextval, systimestamp,
				''Sp_MonitorTask'',  b_SqlCode, b_SqlErrm,
				''Other error'',
				''p_TaskName: '' || p_TaskName || '' | p_TableName: ''')||TO_CLOB(' || p_TableName || '' | p_PartName: '' || p_PartName || '' | v_Status: '' || v_Status || '', '' || Fn_StatusNbrToChar(v_Status) );
			commit;
			RAISE;
	END Sp_MonitorTask;
	/***************************************************************************/
...
order by partition_name;
		...
		IS
				select	partition_name
				from	user_tab_partitions
	')||TO_CLOB('			where	table_name = p_TableName
						and partition_name not in
						(
						select	partition_name
						from	purge_history_partitions
	...
	BEGIN
		...
		/********************************************************************/
		dbms_output.put_line(''DEBUG | History Task Status -'' ||v_HistoryRec.STATUS );
		if( v_HistoryRec.STATUS in ( c_Stopped, c_Chunked, c_Crashed, c_FkDelete, c_SubPartDrop, c_FinishedErrors, c_Processed, c_Process')||TO_CLOB('ing, c_Failed ) ) then
dbms_output.put_line(''DEBUG | Restart Stopped task from previous run, task status was -'' || v_HistoryRec.STATUS );
		...
		while v_TableCount < v_EligibleCount loop
		   ...
		   dbms_output.put_line (''DEBUG| Before call First table to purge  TABLE_NAME - '' || v_HistoryRec.TABLE_NAME  );
			...
			if( v_PurgeTable = ''PURGE_HISTORY'' ) then
				...
				for lr in c_parts( v_PurgeTable ) loop
					Sp_PurgePartition( v_PurgeTable, lr.partition_name, v_PurgeColumn, v_HistoryRec.ID, v_HistoryRec.PURGE_DA')||TO_CLOB('TE );
				end loop;
			end if;
	...
	EXCEPTION
		WHEN e_AlreadyRunning THEN
			...
			values( master_seq.nextval, systimestamp,
				...
				''None'' );
			commit;
			Sp_SendStatusReport( ''FAILED: Attempted to start purge when it is already running according to purge_run_check table, exiting.  '' || chr(10) || ''If this is incorrect pl')||TO_CLOB('ease truncate purge_run_check table'', sysdate, c_Yes );
			RAISE_APPLICATION_ERROR( -20299, ''Purge is already running according to purge_run_check table, exiting'' || chr(10) || ''If this is incorrect please truncate purge_run_check table'' );
		WHEN e_SubPartDrop THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
		...
		WHEN e_FinishedErrors THEN
			...
			update purge_history set row = v_HistoryRec where id = v_HistoryRec.ID;
			commit;
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, excepti')||TO_CLOB('on_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Sp_PurgeSBMO'',  b_SqlCode, b_SqlErrm,
				''Finished with errors.'',
				''None'' );
		...
		WHEN e_BlackoutDay THEN
			...
			v_HistoryRec.Status := c_Stopped;
			v_HistoryRec.TOT_ELAPSED_SECS := Fn_GetTaskElapsedSecs( v_HistoryRec.ID );
			update purge_history set row = v_Histo')||TO_CLOB('ryRec where id = v_HistoryRec.ID;
			commit;
			insert into purge_error_log(id, error_time, code_block, sql_code, sql_errm, exception_message, parameter_values)
			values( master_seq.nextval, systimestamp,
				''Sp_PurgeSBMO'',  b_SqlCode, b_SqlErrm,
		...
		WHEN e_FailedTask THEN
			...
			commit;
			Sp_SendStatusReport( ''FAILED: Failed Task'', v_ResumeTime, c_Yes );
			RAISE_APPLICATION_ERROR( -20200, ''Task failed. Get errors from purge_error_log using: select * from table(pkg_sbmo_purge.fn_ShowErrors('' || b_ErrDate || '')).  Also review dba_parallel_exec')||TO_CLOB('ute_tasks.'' );
		WHEN OTHERS THEN
			b_SqlCode := SQLCODE;
			b_SqlErrm := substr(SQLERRM,1,250);
			DBMS_OUTPUT.PUT_LINE( ''Error in Sp_PurgeSBMO'' );
...
END Pkg_Sbmo_Purge;			-- End Package Body
')),
	 ('sbmo.package.rotate_partitions_pkg.sql',TO_CLOB('
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SBMO"."ROTATE_PARTITIONS_PKG" as
  ...
  BEGIN
    IF p_sql IS NOT NULL THEN
      insert into partition_sql_history (id, sql_timestamp, sql_text)
          values (master_seq.nextval, systimesta')||TO_CLOB('mp, p_sql)
          returning id into l_id;
      commit;
      execute immediate p_sql;
      commit;
  ...
  procedure ')||TO_CLOB('export_subpartition(p_table_name varchar2, p_subpartition_name varchar2)
  is
...
begin
  ...
  dbms_datapump.add_file (han')||TO_CLOB('dle => h1, filename => l_filename || ''.dmp'', directory => l_dir, filetype => 1);
  ...
  LOOP
    ...
    js := sts.job_status;
    percent_done := js.percent_done;')||TO_CLOB('
  END LOOP;
         ...
         IF l_hd < l_d THEN
            IF export_partitions THEN
               IF check_changes (r1.table_name, p1.subpartition_name) THEN
                  ...
                  l_s := l_s || ''SUBPARTITION ('' || p1.su')||TO_CLOB('bpartition_name || '')'';
                  apply_changes (l_s);
  ...
  begin
    ...
    l_future := lookup_properties(''createFuturePartitionsMon')||TO_CLOB('ths'');
    l_past := lookup_properties(''dataRetentionMonths'');
    ...
    dbms_lob.append( l_msg_html, ''<th align="center";>')||TO_CLOB('ID'' || ''</th>''  || chr(10));
    ...
    if l_send_email = ''Y'' then
	 ...
	 ('sbmo.package_body.rotate_partitions_pkg.sql',TO_CLOB('
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "SBMO"."ROTATE_PARTITIONS_PKG" as
  ...
  l_filename  varchar2(250);
  l_rand varchar2 (50);
 ')||TO_CLOB(' l_username varchar2(30);
  l_dir sbmo_properties.value%type;
  l_logdir sbmo_properties.value%type;
  h1          number;
  percent_done number;
...
begin
  ...
  dbms_datapump.set_parallel(handle => h1, degree =')||TO_CLOB('> 4);
  ...
  -- check log file for errors.
  UTL_FILE.fgetattr(l_logdir, l_filename || ''.l')||TO_CLOB('og'', l_exists, l_file_length, l_blocksize);
         ...
         IF l_hd < l_d THEN
            IF export_partitions THEN
               IF check_changes (r1.table_name, p1.subpartition_name) THEN
                  ...
                  apply_changes (l_s);
                  l_s := ''alter table '' || r1.table_name || '' '' ;
   ')||TO_CLOB('               l_s := l_s || ''DROP SUBPARTITION '' || p1.subpartition_name ;
                  apply_changes (l_s);
                  l_s := null;
               END IF;
            ELSE
  ...
  begin
    ...
    l_past := lookup_properties(''dataRetentionMonths'');
    for r1 in (select p.table_name, u.partition_name, u.tablespace_na')||TO_CLOB('me
                   from partition_table_list p, user_tab_partitions u
                   where p.table_name = u.table_name
                   order by p.table_rank, u.partition_name)
    loop
    ...
    dbms_lob.append( l_msg_html, ''<th align="center";>Timestamp'' || ''</th>''  || chr(10));
    dbms_lob.')||TO_CLOB('append( l_msg_html, ''<th align="center";>SQL'' || ''</th>''  || chr(10));
    dbms_lob.append( l_msg_html, ''<th align="center";>Error'' || ''</th>''  || chr(10));
    dbms_lob.append( l_msg_html, ''</tr>''  || chr(10));
    for r in (select id, sql_timestamp, sql_text, sql_error
                   from partition_sql_history
